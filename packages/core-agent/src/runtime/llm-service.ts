...
import type { FinishEvent, StreamEvent, StreamFromOptions, StreamOptions } from '../types/llm'

import { stepCountAtLeast } from '@xsai/shared-chat'
import { streamText } from '@xsai/stream-text'

import type { CachedCompatibility } from '../types/llm'

import { errorMessageFromValue } from '../utils/error-message'

// NOTICE:
// TTL for compatibility cache entries. After this duration, the auto-degrade
// decision is reset, allowing the provider to be retried with full capabilities.
// This prevents permanent capability loss from transient errors (e.g., rate
// limits that contain tool-related keywords).
const COMPATIBILITY_CACHE_TTL = 30 * 60 * 1000 // 30 minutes

/**
 * Check if a cached compatibility entry is still valid (not expired).
 */
function isCacheValid(entry: CachedCompatibility): boolean {
  if (!entry.timestamp) return true // No timestamp means no TTL, always valid
  return Date.now() - entry.timestamp < COMPATIBILITY_CACHE_TTL
}
...
export function streamOptionsToolsCompatibilityOk(model: string, chatProvider: ChatProvider, options?: StreamOptions): boolean {
  ...
  const cached = options?.toolsCompatibility?.get(key)
  if (cached !== undefined) {
    // Handle both legacy boolean format and new CachedCompatibility format
    if (typeof cached === 'boolean') {
      return cached
    }
    // CachedCompatibility with TTL
    if (!isCacheValid(cached)) {
      // Entry expired, remove it and allow retry
      options?.toolsCompatibility?.delete(key)
      return true
    }
...
export function streamOptionsContentArrayCompatibilityOk(model: string, chatProvider: ChatProvider, options?: StreamOptions): boolean {
  ...
  // Check cache with TTL support
  const cached = options?.contentArrayCompatibility?.get(key)
  if (cached !== undefined) {
    // Handle both legacy boolean format and new CachedCompatibility format
    if (typeof cached === 'boolean') {
      return cached
    ...
    if (!isCacheValid(cached)) {
      // Entry expired, remove it and allow retry
      options?.contentArrayCompatibility?.delete(key)
      return true
    }
    return cached.value !== false
  }
...
async function resolveTools(options?: StreamOptions, signal?: AbortSignal) {
  const tools = typeof options?.tools === 'function'
    ? await options.tools({ signal })
    : options?.tools
  return tools ?? []
}
...
function resolveCapturedToolErrorEvent(
  ...
  if (e?.type !== 'tool-result' || typeof e.toolCallId !== 'string')
    return event

  const result = capturedToolErrorByCallId.get(e.toolCallId)
  if (result == null)
    return event

  capturedToolErrorByCallId.delete(e.toolCallId)
  return {
    ...e,
    type: 'tool-error',
    isError: true,
...
export async function streamFrom({
  ...
  const builtinTools = supportedTools
    ...
    : []
  const customTools = supportedTools ? await resolveTools(options, options?.abortSignal) : []
      ...
      try {
        ...
        if (event && typeof event === 'object' && 'type' in event) {
          ...
          if (typedEvent.type === 'finish') {
            ...
            rejectOnce(errorEvent.error ?? new Error('Stream error'))
          }
        }
      }
      catch (error) {
