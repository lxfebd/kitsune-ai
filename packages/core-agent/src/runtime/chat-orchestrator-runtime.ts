...
function cloneStreamingMessage(message: StreamingAssistantMessage): StreamingAssistantMessage {
  try {
    ...
    // Fallback for environments/objects where structuredClone is unavailable
    // or the message graph contains non-cloneable members (functions, etc.).
...
export function createChatOrchestratorRuntime(deps: ChatOrchestratorRuntimeDeps): ChatOrchestratorRuntime {
  ...
  let activeSend: QueuedSend | null = null
    ...
    handlers: [
      async ({ data }) => {
        ...
        if (cancelled || abortController.signal.aborted) {
          deferred.reject(new Error('Send was cancelled before execution'))
          return
        }
  ...
  function cancelPendingSends(sessionId?: string) {
    ...
    // Also abort the active send if it belongs to the target session
