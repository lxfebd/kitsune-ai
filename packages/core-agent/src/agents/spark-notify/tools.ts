import type { ContextUpdate } from '@kitsune/server-sdk'
...
function normalizeSparkNotifyCommand(
  ...
  return {
    ...
    guidance: command.guidance
      ...
      : undefined,
    // NOTICE: contexts placeholder for future use
    contexts: [],
...
export async function createSparkNotifyTools(options: CreateSparkNotifyToolsOptions) {
  ...
  const sparkNoResponseTool = rawTool({
    ...
    execute: async (_rawPayload, context) => {
      ...
      return 'Kitsune System: Acknowledged, no response or action will be processed.'
    },
  })
  if (allowNoResponse)
  ...
  const sparkCommandTool = rawTool({
    ...
    execute: async (rawPayload, context) => {
      ...
      try {
        ...
        return `Kitsune System: Error - invalid spark_command parameters: ${errorMessageFrom(error)}`
      }

      return 'Kitsune System: Acknowledged, command fired.'
    },
  })
