...
} from '@kitsune/plugin-protocol/types'
 ...
 */
export function createHostDataValueSchema(): BaseSchema<HostDataValue, HostDataValue, BaseIssue<unknown>> {
  return union([
    null_(),
    string(),
    boolean(),
    ...
    pipe(record(string(), lazy(createHostDataValueSchema)), check((input: Record<string, HostDataValue>) => isPlainObject(input))),
  ])
}
 ...
 */
export const hostDataRecordSchema = pipe(record(string(), lazy(createHostDataValueSchema)), check((input: Record<string, HostDataValue>) => isPlainObject(input)))
...
export interface ExtensionManifestV1 {
  ...
  kind: 'manifest.extension.kitsune.ai'
 ...
 *
 * Use when:
 * - Parsing `extension.kitsune.json` before loading an extension into the host
 *
 * Expects:
...
export const extensionManifestV1Schema = object({
  ...
  kind: literal('manifest.extension.kitsune.ai'),
  id: string(),
