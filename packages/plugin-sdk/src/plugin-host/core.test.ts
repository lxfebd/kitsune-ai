...
describe('extension manifest schema', () => {
  it('accepts extension.kitsune.json v1 manifests', () => {
  ...
  it('rejects legacy extension manifests', () => {
    const result = safeParse(extensionManifestV1Schema, {
      apiVersion: 'v1',
      kind: 'manifest.plugin.kitsune.ai',
      name: 'kitsune-plugin-test',
      permissions: {},
      entrypoints: {
        electron: './plugin.mjs',
      },
...
describe('for ExtensionHost', () => {
    ...
    await expect(host.startExtension(extension, {
      manifest: {
        ...
        permissions: {
          ...
          resources: [
            { key: 'kitsune:plugin-sdk:resources:kits:kit.cleanup-failure:bindings', actions: ['write'] },
          ],
        },
        entrypoints: {},
      },
  ...
  it('cleans up module-scoped kit resources when the module is disposed', async () => {
    ...
    const permissions: ModulePermissionDeclaration = {
      ...
      resources: [
        { key: 'kitsune:plugin-sdk:resources:kits:kit.module-dispose:bindings', actions: ['write'] },
      ],
    }
    const extension = defineExtension({
...
describe('for FileSystemLoader', () => {
  const testPermissions: ModulePermissionDeclaration = {
    apis: [
      ...
      { key: 'kitsune:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['invoke'] },
    ],
    resources: [
      { key: 'kitsune:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['read'] },
    ],
    capabilities: [
      { key: 'kitsune:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['wait'] },
    ],
  }
