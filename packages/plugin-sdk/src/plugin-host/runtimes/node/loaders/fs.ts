...
export class FileSystemLoader {
  ...
  async loadExtensionFor(manifest: ExtensionManifestV1, options?: ExtensionLoadOptions) {
    ...
    // Windows requires file:// URLs for dynamic ESM imports of absolute paths
    const importUrl = isAbsolute(entrypoint) ? pathToFileURL(entrypoint).href : entrypoint
    const extensionModule = await import(importUrl)
