...
const props = withDefaults(defineProps<{
  ...
  paused?: boolean
  /**
   * Maximum render frames-per-second for the VRM renderer.
   *
   * Passed straight to TresJS `<TresCanvas :fps-limit>`. `0` / negative means
   * unlimited (TresCanvas treats a missing `fpsLimit` as `Infinity`), so we map
   * it to `undefined` here. A capped value keeps the idle breathing/blink
   * animation smooth while avoiding full-refresh-rate WebGL redraws.
   */
  maxFps?: number
}>(), {
...
/**
 * Maps a user-facing max-FPS value into a TresCanvas `fpsLimit`:
 * - `0` / undefined / negative → `undefined` (TresCanvas falls back to no cap)
 * - positive → rounded to at least 1
 */
function resolveThreeFpsLimit(limit?: number): number | undefined {
  if (!limit || limit <= 0)
    return undefined
...
// Resume the on-demand render loop when the page becomes visible again.
function handleThreeVisibilityChange() {
  if (!document.hidden && tresContextRef.value)
    // NOTICE: In @tresjs/core 5.x, invalidate() moved from TresContext top-level
    // to the renderer sub-object. The old `tresContextRef.value.invalidate()`
    // call throws "is not a function" on 5.8.0.
    tresContextRef.value.renderer.invalidate()
}

function onTresRender() {
  // In `on-demand` render mode the canvas only draws when invalidated.
  // Keep the loop alive while the page is visible so idle animation / damping
  // keep advancing at the configured `fps-limit`; when hidden we stop
  // scheduling so the WebGL draw loop fully halts (mirrors Spine's
...
onMounted(() => {
  ...
  // When the page becomes visible again after being hidden, the on-demand
  // render loop has halted (no frames → no `@render` to self-schedule), so we
  // kick a single frame to resume it.
  document.addEventListener('visibilitychange', handleThreeVisibilityChange)
})

onUnmounted(() => {
  document.removeEventListener('visibilitychange', handleThreeVisibilityChange)
  invalidateBindingRevision()
...
<template>
  <Screen ref="screenRef" v-slot="{ width, height }" relative>
    <TresCanvas
      ...
      :clear-alpha="0"
      :fps-limit="resolveThreeFpsLimit(props.maxFps)"
      @ready="onTresReady"
      @render="onTresRender"
    >
      <OrbitControls
