...
- `@kitsune/stage-ui-three/trace`: local Eventa trace bus and snapshot helpers.
- `@kitsune/stage-ui-three/composables/vrm`: VRM loading and animation helpers.
- `@kitsune/stage-ui-three/utils/vrm-preview`: one-off VRM preview rendering.
...
`@kitsune/stage-ui-three/trace` provides:
...
- Use `@kitsune/stage-ui-three/trace` when you need Three/VRM runtime telemetry without routing through Vue parent chains.
