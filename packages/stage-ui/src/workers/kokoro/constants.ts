 ...
 */
export function getDefaultKokoroModel(hasWebGPU: boolean, _fp16Supported?: boolean): KokoroQuantization {
  // NOTICE: This is a desktop-pet product, so TTS must keep its VRAM tiny too.
  // On WebGPU we default to INT8 (q8): ~82 MB (half of the old fp16-webgpu's
  // 165 MB) with negligible quality loss — Kokoro-82M is small enough that q8
  // is transparent to listeners. On CPU/WASM we keep the lightest q4f16
  // (~41 MB). fp16-webgpu / fp32-webgpu stay selectable for users who want max
  // quality and have spare GPU memory.
  if (hasWebGPU) {
    return 'q8-webgpu'
  }
  return 'q4f16'
