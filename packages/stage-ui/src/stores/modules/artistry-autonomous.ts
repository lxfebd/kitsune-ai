...
import { artistryGenerateHeadless } from '@kitsune/stage-shared'
...
// Module-level lazy singleton — avoids creating a new eventa context on every call.
let cachedIpc: { generate: any, addWidget: any } | null | undefined

function getGenerateHeadless() {
  if (cachedIpc !== undefined)
    return cachedIpc
...
export const useAutonomousArtistryStore = defineStore('artistry-autonomous', () => {
  ...
  async function runArtistTask(inputText: string, history: Message[] = [], targetOverride?: 'user' | 'assistant') {
    ...
    const artistry = activeCard?.extensions?.kitsune?.modules?.artistry
...
"${inputText}"`
      ...
      const modelId = activeModelStore.activeModel
      const providerId = activeModelStore.activeProvider
      ...
      if (analysis.intensity >= threshold) {
        ...
        const result = await invokers.generate(toRaw(generationPayload))
            ...
            case 'bg':
              ...
              cardStore.updateCard(cardId, {
                extensions: {
                  ...
                  kitsune: {
                    ...activeCard.extensions.kitsune,
                    modules: {
                      ...activeCard.extensions.kitsune.modules,
