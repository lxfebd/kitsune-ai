...
// In Electron dev, Vite serves large zip files via /@fs/ which often fails
// with partial-content errors. Use a dedicated file-server route instead.
// The main process starts a lightweight HTTP server on this port.
...
const presetLive2dProUrl = live2dModelUrl('hiyori_pro_zh.zip')
const presetLive2dFreeUrl = live2dModelUrl('hiyori_free_zh.zip')
...
export const useDisplayModelsStore = defineStore('display-models', () => {
  ...
  async function initialize() {
    await import('@kitsune/stage-ui-live2d/utils/live2d-zip-loader')
    await import('@kitsune/stage-ui-live2d/utils/live2d-opfs-registration')

    const { loadLive2DModelPreview } = await import('@kitsune/stage-ui-live2d/utils/live2d-preview')
    const { loadVrmModelPreview } = await import('@kitsune/stage-ui-three/utils/vrm-preview')
    const { loadSpineModelPreview } = await import('@kitsune/stage-ui-spine/utils/spine-preview')
