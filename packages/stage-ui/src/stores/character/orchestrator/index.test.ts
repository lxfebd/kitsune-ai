...
import { usePersonaStore, useActiveModelStore } from '../../modules'
...
describe('store character-orchestrator', () => {
  beforeEach(() => {
    ...
    const activeModelStore = useActiveModelStore(pinia)
    activeModelStore.activeProvider = 'mock-provider'
    activeModelStore.activeModel = 'mock-model'

    const kitsuneCardStore = usePersonaStore(pinia)
    // @ts-expect-error - testing purpose
    kitsuneCardStore.systemPrompt = 'You are a brave adventurer in Minecraft.'
    // @ts-expect-error - testing purpose
    kitsuneCardStore.activeCard = {
      name: 'Hero',
      version: '1.0',
      extensions: {
        kitsune: {
  ...
  it('supports forcing text-only spark:notify responses', async () => {
    ...
    const event: WebSocketEventOf<'spark:notify'> = {
      type: 'spark:notify',
      source: 'plugin:kitsune-plugin-game-chess',
      data: {
        id: nanoid(),
        eventId: nanoid(),
        kind: 'ping',
