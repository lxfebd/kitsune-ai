...
import { isStageTamagotchi, isStageWeb } from '@kitsune/stage-shared'
...
// NOTICE: callback uses WebSocketBaseEvent<any, any> for type erasure.
// This is needed because onEvent/offEvent are generic on E, but heterogeneous
// listener arrays can't preserve per-entry type parameters.
// The Client itself does the same erasure internally (client.ts:316).
type TypeErasedEventCallback = (event: WebSocketBaseEvent<any, any>) => void | Promise<void>

interface ChannelListenerEntry {
  type: keyof WebSocketEvents
  callback: TypeErasedEventCallback
  boundClient?: Client
}
...
export const useModsServerChannelStore = defineStore('mods:channels:kitsune:server', () => {
  ...
  const defaultWebSocketUrl = import.meta.env.VITE_KITSUNE_WS_URL || 'ws://localhost:6121/ws'
  ...
  function clearListeners() {
    for (const listener of registeredListeners) {
      if (listener.boundClient) {
        listener.boundClient.offEvent(listener.type, listener.callback as TypeErasedEventCallback)
  ...
  function initializeListeners() {
    ...
    for (const listener of registeredListeners) {
      ...
      listener.boundClient?.offEvent(listener.type, listener.callback as TypeErasedEventCallback)
      client.value.onEvent(listener.type, listener.callback as TypeErasedEventCallback)
  ...
  function registerListener<E extends keyof WebSocketEvents>(
    ...
    const entry: ChannelListenerEntry = {
      type,
      callback: callback as TypeErasedEventCallback,
    }
