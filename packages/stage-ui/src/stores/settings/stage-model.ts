...
export const useSettingsStageModel = defineStore('settings-stage-model', () => {
  ...
  let lastProcessedModelId: string | undefined
  ...
  async function updateStageModel() {
    ...
    // Skip if the same model is already loaded (prevents unnecessary blob URL
    // recreation which triggers Live2D full reload and "Loading..." flash).
    if (selectedModelId === lastProcessedModelId && stageModelSelectedUrl.value) {
      return
    }

    if (!selectedModelId) {
      lastProcessedModelId = undefined
    ...
    if (!model) {
      lastProcessedModelId = selectedModelId
      replaceStageModelUrl(undefined)
      stageModelSelectedDisplayModel.value = undefined
      stageModelBuiltInRenderer.value = 'disabled'
      if (stageModelRenderer.value !== 'godot')
