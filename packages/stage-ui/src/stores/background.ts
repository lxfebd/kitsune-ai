...
export const useBackgroundStore = defineStore('background-entries', () => {
  ...
  const { data: syncSignal, post: broadcastSync } = useBroadcastChannel({ name: 'kitsune:background-sync' })
  ...
  const activeBackgroundUrl = computed(() => {
    ...
    if (!personaStore.activeCard)
      return null
    const bgId = personaStore.activeCard.extensions?.kitsune?.modules?.activeBackgroundId
    if (!bgId || bgId === 'none') {
      return null
    }
  ...
  const availableBackgrounds = computed(() => {
    const personaStore = usePersonaStore()
    return getCharacterBackgrounds.value(personaStore.activeCardId)
  })
  ...
  const journalEntries = computed(() => {
    const personaStore = usePersonaStore()
    return getCharacterJournalEntries.value(personaStore.activeCardId)
  })
  ...
  async function addBackground(
    ...
    const resolvedCharacterId = characterId !== undefined
      ? characterId
      : ((type === 'journal' || type === 'selfie') ? personaStore.activeCardId : null)
