...
// NOTICE: Silero VAD's recommended operating point is ~0.5. The previous 0.6
// was too high: with echo cancellation / noise suppression / AGC enabled on the
// mic, the captured level is conservative, so 0.6 frequently failed to cross the
// threshold — the recognizer never started (pet "can't hear" the user) and when
// it did fire it only caught loud peaks (poor accuracy). 0.5 restores reliable
// speech detection for normal conversational volume.
