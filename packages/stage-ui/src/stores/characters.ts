...
export function createCharacterStoreController(params: {
  ...
  async function update(id: string, payload: UpdateCharacterPayload) {
    ...
    const localCharacter = {
      ...
      ...(payload.prompts !== undefined ? {
        // Replace prompts wholesale with the submitted set; the model/upsert
        // layer expects full rows, so mint stable ids for the optimistic copy.
        prompts: payload.prompts.map((p, index) => ({
