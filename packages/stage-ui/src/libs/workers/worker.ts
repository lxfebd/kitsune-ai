...
// NOTICE: 国内网络无法直接访问 huggingface.co，且 hf-mirror.com 不稳定。
// @huggingface/transformers 使用 env.remoteHost 控制模型下载源。
// Electron 主进程的 model-file-server 在 localhost:19528 提供本地模型文件。
// Removal condition: 当 huggingface.co 在国内可正常访问时可移除。
env.remoteHost = 'http://localhost:19528'
...
class AutomaticSpeechRecognitionPipeline {
  ...
  static async getInstance(progress_callback?: ProgressCallback, device: 'webgpu' | 'wasm' | 'cpu' = 'webgpu', modelId?: string) {
    // Support switching the underlying HF model (e.g. whisper-tiny vs small).
    // Reset cached singletons when the requested model differs so the new
    // model is fetched and compiled instead of reusing the previous one.
    const nextModelId = modelId ?? this.model_id ?? MODEL_ID
    if (this.model_id !== nextModelId) {
      this.model_id = nextModelId
      this.model = null
      this.tokenizer = null
      this.processor = null
    }
    ...
    // NOTICE: dtype is device-specific, and VRAM footprint is the priority for
    // this desktop-pet product — we never want the multi-GB WebGPU blow-up that
    // the old fp16 encoder caused (whisper-small inflated to 7 GB+ on an 8 GB
    // laptop dGPU). So:
    // - WebGPU: INT8 (q8) encoder + q4 decoder. q8 halves the encoder's memory
    //   vs fp16 and keeps the intermediate-activation buffer far smaller, while
    //   still running entirely on the GPU. fp16 decoder is known broken (Issue
    //   #989), so the decoder stays q4.
    //   https://github.com/huggingface/transformers.js/issues/989
    // - WASM/CPU (no dGPU, low-end machines): q8 encoder + q8 decoder keeps the
    //   model at ~1/4 the fp32 size and speeds up CPU inference 2-4x with
    //   negligible accuracy loss — the lever that makes local ASR viable
    ...
    this.model ??= (async () => {
      ...
      for (const dtype of dtypeAttempts) {
        try {
          ...
          lastError = error
          console.warn(
            `[Whisper Worker] dtype ${JSON.stringify(dtype)} failed on ${actualDevice}, trying next:`,
  ...
  /**
   * Actually unload the model from memory and GPU.
   * Clears all static fields to allow garbage collection.
   */
  static unload(): void {
    if (this.model) {
      this.model.then((m) => {
    ...
    this.processor = null
    this.model_id = null
    console.log('[Whisper Worker] Model unloaded from memory')
  }
}
...
async function loadModel(request: LoadModelRequest): Promise<void> {
  ...
  try {
    ...
    const [_tokenizer, _processor, model] = await AutomaticSpeechRecognitionPipeline.getInstance((x: any) => {
      ...
      // NOTICE: @huggingface/transformers progress_callback already sends progress as 0-100
      // (see bundled transformers.js: `loaded / total * 100`), no additional scaling needed.
      if (currentLoadRequestId) {
        if (x.status === 'progress') {
          sendProgress(currentLoadRequestId, 'download', x.progress ?? -1, undefined, {
            file: x.file,
            loaded: x.loaded,
            total: x.total,
          })
    ...
    }, device as 'webgpu' | 'wasm' | 'cpu', request.modelId)
    ...
    // NOTICE: Warmup must use the model's expected input shape.
    // whisper-small uses num_mel_bins=80 (not 128 like large-v3-turbo).
    // Using wrong dimensions causes OrtRun dimension errors.
...
async function runInference(request: RunInferenceRequest<WhisperInput>): Promise<void> {
  ...
  try {
    ...
    // NOTICE: Only pass `language` if it's a valid language name (not undefined/'auto').
    // The ONNX Whisper model rejects 'auto' and requires explicit language names
    // like "chinese", "english", etc. When omitted, whisper-small may output
    // "(speaking in Mandarin)" instead of actual text for short Chinese audio.
    // Default to "chinese" when no language is specified for better CJK recognition.
    if (input.language && input.language !== 'auto')
      generateOptions.language = input.language
    else
      generateOptions.language = 'chinese'
...
globalThis.addEventListener('message', async (event: MessageEvent<WorkerInboundMessage<WhisperInput>>) => {
  ...
  switch (message.type) {
    ...
    case 'unload-model':
      // Actually unload the model from memory and GPU
      AutomaticSpeechRecognitionPipeline.unload()
