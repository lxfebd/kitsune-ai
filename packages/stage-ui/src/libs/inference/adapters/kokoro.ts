...
/**
 * Pets repeat the same greetings / phrases constantly. Caching generated WAV
 * audio by (voice|text) lets identical utterances skip Kokoro inference
 * entirely — the dominant TTS cost. Bounded LRU caps memory footprint.
 */
const TTS_PHRASE_CACHE_MAX = 200
const ttsPhraseCache = new Map<string, ArrayBuffer>()

function ttsCacheKey(text: string, voice: VoiceKey): string {
  return `${voice} ${text}`
}
...
export function createKokoroAdapter(): KokoroAdapter {
      ...
      return getLoadQueue().enqueue(modelStatusId, LOAD_PRIORITY.TTS, async () => {
        ...
        const readyPromise = waitForWorkerMessage<any>(worker!, requestId, 'model-ready', LOAD_MODEL_TIMEOUT, (data) => {
          if (data.type === 'progress') {
            ...
            // 根据进度区分下载和加载阶段
            const newState = (payload.percent ?? -1) >= 100 ? 'loading' : 'downloading'
            updateInferenceStatus(modelStatusId, { state: newState, progress })
        ...
        const estimated = MODEL_VRAM_ESTIMATES[estimateKey] ?? 82 * 1024 * 1024
  ...
  async function generate(
    ...
    // Fast path: return a private copy of a previously synthesized phrase so
    // the worker (and Kokoro inference) is skipped entirely.
    const key = ttsCacheKey(text, voice)
    const cached = ttsPhraseCache.get(key)
    if (cached) {
      ttsPhraseCache.delete(key)
      ttsPhraseCache.set(key, cached)
    ...
    return defaultPerfTracer.withMeasure('inference', 'kokoro-generate', () => operationMutex.runExclusive(async () => {
      ...
      if (output.action === 'generate') {
        state = 'ready'
        onSuccess()
        const wav = encodeWav(output.samples as Float32Array, output.samplingRate as number)
        // Cache the freshly synthesized buffer (callers receive an owned copy).
        ttsPhraseCache.delete(key)
        ttsPhraseCache.set(key, wav)
        if (ttsPhraseCache.size > TTS_PHRASE_CACHE_MAX) {
          const oldest = ttsPhraseCache.keys().next().value
          if (oldest !== undefined)
            ttsPhraseCache.delete(oldest)
        }
        return wav
      }
