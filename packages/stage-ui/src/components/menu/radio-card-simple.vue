...
<template>
  <label
    ...
    :class="[
      modelValue === value
        ? 'bg-primary-50/80 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800/50 backdrop-blur-xl'
        : 'bg-white/70 dark:bg-neutral-900/30 backdrop-blur-xl border-black/5 dark:border-white/5',
      modelValue === value
        ? 'form_radio-card-simple-active'
        : '',
    ]"
    flex="~ col"
    block min-w-50 w-full h-full cursor-pointer items-start rounded-xl p-4 text-left
  >
    <input
      v-model="modelValue"
