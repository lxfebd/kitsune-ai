...
<template>
  <div
    flex="~ col"
    bg="white/60 dark:neutral-900/30 backdrop-blur-xl"
    border="black/5 dark:white/5 hover:primary-500/30 dark:hover:primary-400/30 solid 1"
    class="menu-icon-status-item"
    transition="all duration-200 ease-in-out"
    h-full w-full cursor-pointer of-hidden rounded-2xl active:scale-[0.98]
    hover="shadow-md shadow-black/5 dark:shadow-black/10"
  >
    <RouterLink
      flex="~ row"
      class="menu-icon-status-item-link"
      bg="white/40 dark:neutral-900/20"
      transition="all duration-200 ease-in-out"
      relative h-full w-full items-center overflow-hidden rounded-t-2xl p-5 text-left
      :to=" props.to"
    >
      <div z-1 flex-1>
        <div
        ...
        <div v-if="props.pricing || props.deployment" mt-2 flex flex-wrap gap-1.5>
          <div
            v-if="props.pricing"
