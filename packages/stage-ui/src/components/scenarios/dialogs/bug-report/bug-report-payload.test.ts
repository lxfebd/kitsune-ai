...
describe('bug report payload helpers', () => {
  it('builds markdown payload with description and optional context/screenshot flags', () => {
    const payload = buildBugReportPayload({
      ...
      context: {
        url: 'https://kitsune.local/chat?room=debug',
        title: 'AIRI Chat',
    ...
    expect(payload).toContain('- URL: https://kitsune.local/chat?room=debug')
  ...
  it('extracts page context from a window-like object', () => {
    const context = createBugReportPageContext({
      location: {
        href: 'https://kitsune.local/settings?tab=providers',
      },
      document: {
        title: 'Settings',
      },
    ...
    expect(context).toEqual({
      url: 'https://kitsune.local/settings?tab=providers',
      title: 'Settings',
      userAgent: 'unit-test',
      viewport: '1280x720',
      language: 'en-US',
