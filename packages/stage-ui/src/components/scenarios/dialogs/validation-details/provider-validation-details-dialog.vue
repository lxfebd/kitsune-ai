...
<template>
        ...
        <div :class="['flex', 'flex-col', 'gap-3']">
          ...
          >
            ...
            <div :class="['mt-1', 'text-xs', 'text-neutral-600', 'dark:text-neutral-300']">
              {{ step.reason || t('settings.validation-details.validation-failed') }}
            </div>
          </div>
        </div>
      </DialogContent>
  ...
  <DrawerRoot v-else :open="showDialog" should-scale-background @update:open="value => showDialog = value">
    <DrawerPortal>
      ...
      <DrawerContent
        :class="['fixed', 'bottom-0', 'left-0', 'right-0', 'z-1000', 'mt-20', 'h-full', 'max-h-[70%]', 'flex', 'flex-col', 'rounded-t-2xl', 'bg-white/90', 'dark:bg-neutral-900/90', 'px-4', 'pt-4', 'outline-none', 'backdrop-blur-2xl', 'border-t', 'border-black/5', 'dark:border-white/5']"
