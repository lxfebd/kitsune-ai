 ...
 *   message preview as a fallback) and a relative updatedAt timestamp.
 */
...
const { activeCardId } = storeToRefs(usePersonaStore())
const { activeModel } = storeToRefs(useActiveModelStore())
...
/**
 * Sessions visible in the drawer. All sessions are local-only now that
 * login/cloud sync has been removed.
 */
const ownedSessions = computed(() => {
  return Object.values(sessionMetas.value).filter(meta => meta.userId === 'local')
})
