...
import { Button, FieldCombobox, FieldRange } from '@kitsune/ui'
  ...
  <div w-full pt-1>
    ...
    <div mb-2>
      <FieldCombobox
        ...
        :description="t('settings.hearing.audio-input-device-description')"
    ...
    <Button class="my-4" w-full @click="toggleMonitoring">
      {{ isMonitoring ? t('settings.hearing.stop-monitoring') : t('settings.hearing.start-monitoring') }}
    </Button>
    ...
    <div class="space-y-3">
      <!-- Volume Meter -->
      <LevelMeter :level="volumeLevel" :label="t('settings.hearing.input-level')" />

      <!-- VAD Probability Meter (when VAD model is active) -->
      <ThresholdMeter
        :value="volumeLevel / 100"
        :threshold="speakingThreshold / 100"
        :label="t('settings.hearing.speech-probability')"
        :below-label="t('settings.hearing.silence')"
        :above-label="t('settings.hearing.speech')"
        :threshold-label="t('settings.hearing.detection-threshold')"
      />

      <div class="space-y-3">
        <FieldRange
          v-model="speakingThreshold"
          :label="t('settings.hearing.sensitivity')"
          :description="t('settings.hearing.sensitivity-description')"
          :min="1"
          :max="80"
          :step="1"
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="text-sm font-medium">
          {{ isSpeaking ? t('settings.hearing.speaking-detected') : t('settings.hearing.silence') }}
        </span>
      </div>
    </div>
  </div>
