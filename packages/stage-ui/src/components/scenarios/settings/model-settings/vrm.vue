...
import { Button, Callout, SelectTab } from '@kitsune/ui'
...
const { vrmMaxFps } = storeToRefs(useSettingsVrm())
const fpsOptions = computed(() => [
  { value: 30, label: '30' },
  { value: 60, label: '60' },
  { value: 0, label: t('settings.vrm.rendering.fps.options.unlimited') },
])
...
const envOptions = computed(() => [
  {
    value: 'hemisphere',
    label: t('settings.vrm.scale-and-position.hemisphere'),
  ...
  {
    value: 'skyBox',
    label: t('settings.vrm.scale-and-position.skybox'),
...
<template>
  ...
  >
    ...
    <label class="flex flex-wrap items-center gap-4 px-2 pt-2">
      <span class="text-sm font-medium">{{ t('settings.vrm.rendering.max-fps') }}</span>
      <SelectTab v-model="vrmMaxFps" :options="fpsOptions" size="sm" :class="['shrink-0']" />
    </label>
    ...
    <div grid="~ cols-5 gap-1" p-2 :class="settingsLockClass">
      <PropertyPoint
        ...
        :label="t('settings.vrm.scale-and-position.model-position')"
      ...
      <PropertyNumber
        ...
        :label="t('settings.vrm.scale-and-position.directional-light-rotation-x')"
      />
      <PropertyNumber
        ...
        :label="t('settings.vrm.scale-and-position.directional-light-rotation-y')"
      />
      <PropertyColor
        v-model="directionalLightColor"
        :disabled="controlsLocked"
        :label="t('settings.vrm.scale-and-position.directional-light-color')"
      />

      <PropertyNumber
        v-model="directionalLightIntensity"
        :config="{ min: 0, max: 10, step: 0.01, label: 'Intensity', disabled: controlsLocked }"
        :label="t('settings.vrm.scale-and-position.directional-light-intensity')"
      />

      <PropertyNumber
        v-model="ambientLightIntensity"
        :config="{ min: 0, max: 10, step: 0.01, label: 'Intensity', disabled: controlsLocked }"
        :label="t('settings.vrm.scale-and-position.ambient-light-intensity')"
      />
      <PropertyColor
        v-model="ambientLightColor"
        :disabled="controlsLocked"
        :label="t('settings.vrm.scale-and-position.ambient-light-color')"
      />
    </div>
    <div>
      <div
      ...
      >
        {{ t('settings.vrm.scale-and-position.environment') }}
      </div>
      ...
      <div v-if="envSelect === 'hemisphere'">
        ...
        <div grid="~ cols-5 gap-1" p-2 :class="settingsLockClass">
          <PropertyNumber
            ...
            :config="{ min: 0, max: 10, step: 0.01, label: 'Intensity', disabled: controlsLocked }"
            :label="t('settings.vrm.scale-and-position.hemisphere-light-intensity')"
          />
          <PropertyColor
            v-model="hemisphereSkyColor"
            :disabled="controlsLocked"
            :label="t('settings.vrm.scale-and-position.hemisphere-sky-color')"
          />
          <PropertyColor
            v-model="hemisphereGroundColor"
            :disabled="controlsLocked"
            :label="t('settings.vrm.scale-and-position.hemisphere-ground-color')"
          />
        </div>
      </div>
      <div v-else>
  ...
  >
    ...
    <Callout
      theme="lime"
      :label="t('settings.vrm.scale-and-position.tips-title')"
    >
      <div class="text-sm text-neutral-600 space-y-1 dark:text-neutral-400">
