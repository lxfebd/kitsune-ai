...
import { defaultModelParameters, useExpressionStore, useLive2dParams, useSettingsLive2d } from '@kitsune/stage-ui-live2d'
import { OPFSCache } from '@kitsune/stage-ui-live2d/utils/opfs-loader'
import { Button, Checkbox, FieldCheckbox, FieldCombobox, FieldRange, SelectTab } from '@kitsune/ui'
...
<template>
    ...
    <FieldRange v-model="scale" as="div" :min="0.1" :max="3" :step="0.01" :label="t('settings.live2d.scale-and-position.scale')">
      <template #label>
        <div flex items-center>
          ...
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => scale = 1">
    ...
    <FieldRange v-model="position.x" as="div" :min="-3000" :max="3000" :step="1" :label="t('settings.live2d.scale-and-position.x')">
      <template #label>
        <div flex items-center>
          ...
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => position.x = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="position.y" as="div" :min="-3000" :max="3000" :step="1" :label="t('settings.live2d.scale-and-position.y')">
      <template #label>
        <div flex items-center>
          ...
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => position.y = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
  ...
  >
    ...
    <FieldRange v-model="modelParameters.angleX" as="div" :min="-30" :max="30" :step="0.1" label="Angle X">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.angle-x') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.angleX = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.angleY" as="div" :min="-30" :max="30" :step="0.1" label="Angle Y">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.angle-y') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.angleY = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.angleZ" as="div" :min="-30" :max="30" :step="0.1" label="Angle Z">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.angle-z') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.angleZ = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyeOpen" as="div" :min="0" :max="1" :step="0.01" label="Left Eye Open/Close">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eye-open') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyeOpen = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyeOpen" as="div" :min="0" :max="1" :step="0.01" label="Right Eye Open/Close">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eye-open') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyeOpen = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyeSmile" as="div" :min="0" :max="1" :step="0.01" label="Left Eye Smiling">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eye-smile') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyeSmile = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyeSmile" as="div" :min="0" :max="1" :step="0.01" label="Right Eye Smiling">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eye-smile') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyeSmile = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyebrowLR" as="div" :min="-1" :max="1" :step="0.01" label="Left eyebrow Left/Right">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eyebrow-lr') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyebrowLR = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyebrowLR" as="div" :min="-1" :max="1" :step="0.01" label="Right eyebrow Left/Right">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eyebrow-lr') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyebrowLR = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyebrowY" as="div" :min="-1" :max="1" :step="0.01" label="Left Eyebrow Y (Up/Down)">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eyebrow-y') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyebrowY = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyebrowY" as="div" :min="-1" :max="1" :step="0.01" label="Right Eyebrow Y (Up/Down)">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eyebrow-y') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyebrowY = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyebrowAngle" as="div" :min="-1" :max="1" :step="0.01" label="Left Eyebrow Angle">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eyebrow-angle') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyebrowAngle = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyebrowAngle" as="div" :min="-1" :max="1" :step="0.01" label="Right Eyebrow Angle">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eyebrow-angle') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyebrowAngle = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.leftEyebrowForm" as="div" :min="-1" :max="1" :step="0.01" label="Left Eyebrow Form (Deformation)">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.left-eyebrow-form') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.leftEyebrowForm = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.rightEyebrowForm" as="div" :min="-1" :max="1" :step="0.01" label="Right Eyebrow Form (Deformation)">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.right-eyebrow-form') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.rightEyebrowForm = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.mouthOpen" as="div" :min="0" :max="1" :step="0.01" label="Mouth Open/Close">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.mouth-open') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.mouthOpen = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.mouthForm" as="div" :min="-1" :max="1" :step="0.01" label="Mouth Form (Deformation)">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.mouth-form') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.mouthForm = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.cheek" as="div" :min="0" :max="1" :step="0.01" label="Cheek">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.cheek') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.cheek = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.bodyAngleX" as="div" :min="-10" :max="10" :step="0.1" label="Body rotation X">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.body-rotation-x') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.bodyAngleX = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.bodyAngleY" as="div" :min="-10" :max="10" :step="0.1" label="Body rotation Y">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.body-rotation-y') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.bodyAngleY = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.bodyAngleZ" as="div" :min="-10" :max="10" :step="0.1" label="Body rotation Z">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.body-rotation-z') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.bodyAngleZ = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
    ...
    <FieldRange v-model="modelParameters.breath" as="div" :min="0" :max="1" :step="0.01" label="Breath">
      <template #label>
        <div flex items-center>
          <div>{{ t('settings.live2d.parameters.labels.breath') }}</div>
          <button px-2 text-xs outline-none :title="t('settings.live2d.parameters.reset-value')" @click="() => modelParameters.breath = 0">
            <div i-solar:forward-linear transform-scale-x--100 text="neutral-500 dark:neutral-400" />
          </button>
        </div>
      </template>
