import type { ChatProvider, ChatProviderWithExtraOptions, EmbedProvider, EmbedProviderWithExtraOptions, ModelProvider, ModelProviderWithExtraOptions, SpeechProvider, SpeechProviderWithExtraOptions, TranscriptionProvider, TranscriptionProviderWithExtraOptions } from '@xsai-ext/providers/utils';
import type { ProgressInfo } from '@xsai-transformers/shared/types';
import type { MaybePromise } from 'clustr';
import type { ComposerTranslation } from 'vue-i18n';
import type { $ZodType } from 'zod/v4/core';
export type ProviderInstance = ChatProvider | ChatProviderWithExtraOptions | EmbedProvider | EmbedProviderWithExtraOptions | SpeechProvider | SpeechProviderWithExtraOptions | TranscriptionProvider | TranscriptionProviderWithExtraOptions | ModelProvider | ModelProviderWithExtraOptions;
export declare function isModelProvider(providerInstance: ProviderInstance): providerInstance is ModelProvider | ModelProviderWithExtraOptions;
...
export interface ProviderExtraMethods<TConfig> {
    ...
    loadModel?: (config: TConfig, provider: ProviderInstance, hooks?: {
 ...
 */
export declare const CHAT_COMPLETIONS_VALIDATOR_ID = "check-chat-completions";
export declare enum ProviderValidationCheck {
...
export interface ProviderConfigValidator<TConfig> {
    id: string;
    name: string;
    validator: (config: TConfig, contextOptions: {
...
export interface ProviderRuntimeValidator<TConfig> {
    id: string;
    name: string;
    validator: (config: TConfig, provider: ProviderInstance, providerExtra: ProviderExtraMethods<TConfig>, contextOptions: {
        t: ComposerTranslation;
    }) => MaybePromise<ProviderValidationResult>;
    schedule?: ProviderValidatorSchedule;
}
...
export interface ProviderDefinition<TConfig extends any = any> {
    ...
    validators?: {
        validateConfig?: Array<(contextOptions: {
            t: ComposerTranslation;
        }) => ProviderConfigValidator<TConfig>>;
        validateProvider?: Array<(contextOptions: {
            t: ComposerTranslation;
    ...
    capabilities?: {
         ...
         * session adapter selection (`@kitsune/stage-ui/libs/speech/tts-session`):
         *
