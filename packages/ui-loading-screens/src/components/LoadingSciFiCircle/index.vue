...
import CircleFadeInAnimation from './assets/circle_blink_in_-_loading_(@kitsune).riv'
...
const projectKitsuneAsciiArt = computed(() => {
  if (isDeviceSm.value)
...
const bootMessages = computed<BootMessage[]>(() => [
  ...projectKitsuneAsciiArt.value.split('\n').map(line => ({
    template: line,
    typingSpeed: 1,
    withoutTimestamp: true,
  })),
  {
    template: `Project Kitsune version ${import.meta.env.VITE_KITSUNE_VERSION || '1.0.0'} @ ${import.meta.env.VITE_KITSUNE_COMMIT || '0240602'} build`,
    typingSpeed: 5,
    withoutTimestamp: true,
  },
  ...projectKitsuneMetadata.trim().split('\n').map(line => ({
    template: line,
    typingSpeed: 1,
    withoutTimestamp: true,
  })),
  ...
  {
    template: 'Command line: BOOT_IMAGE=/boot/kitsune.ai root=UUID=io.github.kitsune',
    typingSpeed: 1,
  },
  {
  ...
  {
    template: 'Initializing Kitsune subsystems...',
    typingSpeed: 1,
    pending: true,
    onPendingCheck: () => {
  ...
  {
    template: 'Kitsune core services initialized',
    typingSpeed: 1,
  },
  {
