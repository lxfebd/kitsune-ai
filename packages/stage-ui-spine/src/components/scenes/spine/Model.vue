...
// NOTICE:
// SpineCanvas (spine-webgl) uses its own internal requestAnimationFrame loop
// with no public stop()/start() API — only dispose() permanently halts it.
// We track visibility here so the update/render callbacks can early-return
// when the page is hidden, avoiding animation state advancement and WebGL
// draw calls in the background. The RAF itself still fires (throttled by the
// browser) but does no work; on visible, normal rendering resumes.
let visibilityHidden = false

// Frame-rate throttle for the Spine render loop.
// SpineCanvas (spine-webgl) drives its own internal requestAnimationFrame loop
// with no public stop()/start() API, so we gate the per-frame update + draw
// ourselves using `maxFps`. `props.maxFps <= 0` means unlimited (every RAF).
let lastFrameAt = 0
    ...
    await new Promise<void>((resolve, reject) => {
      const app: SpineCanvasApp = {
        ...
        update: (_sc, delta) => {
          ...
          // Respect the user's max-FPS setting: only advance + allow a draw
          // once the interval has elapsed. When capped, the RAF still fires but
          // we skip work; Spine's `delta` carries the accumulated wall time so
          // animation speed stays correct across skipped frames.
          const fps = props.maxFps
          const now = performance.now()
          frameAllowed = !fps || fps <= 0
            ? true
            : (now - lastFrameAt >= 1000 / fps)
          if (!frameAllowed)
            return
          lastFrameAt = now
...
// 页面隐藏时通过 visibilityHidden 标志暂停 update/render 工作，visible 时恢复
function handleSpineVisibilityChange() {
...
onMounted(async () => {
  ...
  // becomes available.
  document.addEventListener('visibilitychange', handleSpineVisibilityChange)
})

onUnmounted(() => {
  isUnmounted = true
  document.removeEventListener('visibilitychange', handleSpineVisibilityChange)
  disposeSpine()
})
