...
This package owns product-specific Electron scenario definitions only. It depends on `@kitsune/vishot-runner-electron` for:
...
1. Build `@kitsune/stage-tamagotchi`.
2. Run this scenario through `@kitsune/vishot-runner-electron`.
...
When running scenarios through `@kitsune/vishot-runner-electron`, the built Electron app can use a different `userData` profile than `dev:tamagotchi`.
