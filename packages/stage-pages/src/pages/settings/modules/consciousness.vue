...
watch([activeProvider, activeModel], ([provider, model]) => {
  personaStore.updateActiveCardConsciousness({ provider, model })
})
  ...
  <div bg="neutral-50 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-4">
    ...
    <div v-if="activeProvider && supportsModelListing">
      <div flex="~ col gap-4">
        ...
        <!-- Model selection dialog trigger -->
        <template v-else-if="providerModels.length > 0">
          <button
            type="button"
            class="w-full flex items-center justify-between border border-neutral-300 rounded-lg bg-white px-4 py-3 text-left transition-colors dark:border-neutral-700 hover:border-primary-400 dark:bg-neutral-900 dark:hover:border-primary-400"
