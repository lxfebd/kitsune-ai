...
import type { ccv3 } from '@kitsune/ccc'

import { Alert } from '@kitsune/stage-ui/components'
import { usePersonaStore } from '@kitsune/stage-ui/stores/modules/persona'
import { InputFileCard } from '@kitsune/ui'
...
function getModuleShortName(id: string, module: 'consciousness' | 'voice') {
  const card = cards.value.get(id)
  if (!card || !card.extensions?.kitsune?.modules)
    return 'default'

  const kitsuneExt = card.extensions.kitsune.modules

  if (module === 'consciousness') {
    return kitsuneExt.consciousness?.model ? kitsuneExt.consciousness.model.split('-').pop() || 'default' : 'default'
  }
  else if (module === 'voice') {
    return kitsuneExt.speech?.voice_id || 'default'
  }
    ...
    <div flex="~ row" flex-wrap items-center justify-between gap-4>
      ...
      <div class="relative flex flex-row justify-start gap-2 lg:flex-col">
        ...
        <ComboboxSelect
          ...
          :placeholder="t('settings.pages.card.select-sort-option')"
          class="min-w-[150px]"
        />
      </div>
    </div>
