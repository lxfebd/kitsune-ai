...
import type { Card } from '@kitsune/ccc'
import type { AiriExtension } from '@kitsune/stage-ui/stores/modules/persona'

import kebabcase from '@stdlib/string-base-kebabcase'

import { isCustomProvidersDisabled } from '@kitsune/stage-shared'
import { DEFAULT_ARTISTRY_WIDGET_INSTRUCTION } from '@kitsune/stage-ui/constants/prompts/artistry-instruction'
...
import { useSettingsStageModel } from '@kitsune/stage-ui/stores/settings/stage-model'
import { Button, FieldInput, FieldValues } from '@kitsune/ui'
import { ComboboxSelect } from '@kitsune/ui/components/form'
...
const consciousnessStore = useActiveModelStore()
...
const tabs: Tab[] = [
  ...
  { id: 'persona', label: t('settings.pages.card.creation.persona-tab.title'), icon: 'i-solar:heart-bold-duotone' },
    ...
    extensions: {
      ...
      kitsune: {
        modules: {
          ...
          persona: {
            mode: personaMode.value,
            soul: personaSoul.value || undefined,
            identity: personaIdentity.value || undefined,
            addressing: {
              defaultUserTitle: personaAddressingTitle.value || undefined,
              customName: personaAddressingCustomName.value || undefined,
              useCustomFirst: personaAddressingUseCustomFirst.value,
            },
          },
        },
        agents: {},
...
function initializeCard(): Card {
  ...
  const kitsuneExt = existingCard?.extensions?.kitsune as AiriExtensionWithLegacyArtistry | undefined

  // Initialize module selections with fallback logic (handles all cases: create, edit with/without extension)
  selectedConsciousnessProvider.value = kitsuneExt?.modules?.consciousness?.provider || consciousnessProvider.value
  selectedConsciousnessModel.value = kitsuneExt?.modules?.consciousness?.model || defaultConsciousnessModel.value
  selectedVisionProvider.value = kitsuneExt?.modules?.vision?.provider || visionProvider.value
  selectedVisionModel.value = kitsuneExt?.modules?.vision?.model || defaultVisionModel.value
  selectedSpeechProvider.value = kitsuneExt?.modules?.speech?.provider || speechProvider.value
  selectedSpeechModel.value = kitsuneExt?.modules?.speech?.model || defaultSpeechModel.value
  selectedSpeechVoiceId.value = kitsuneExt?.modules?.speech?.voice_id || defaultSpeechVoiceId.value
  selectedDisplayModelId.value = kitsuneExt?.modules?.displayModelId || defaultDisplayModelId.value

  // NOTICE: keep legacy `extensions.kitsune.artistry` fallback so existing cards continue to load.
  const artistrySettings = kitsuneExt?.modules?.artistry || kitsuneExt?.artistry
  ...
  const persona = kitsuneExt?.modules?.persona
  personaMode.value = persona?.mode ?? 'hybrid'
  personaSoul.value = persona?.soul ?? ''
  personaIdentity.value = persona?.identity ?? ''
  personaAddressingTitle.value = persona?.addressing?.defaultUserTitle ?? '主人'
  personaAddressingCustomName.value = persona?.addressing?.customName ?? ''
  personaAddressingUseCustomFirst.value = persona?.addressing?.useCustomFirst ?? true
...
// Persona state (must be defined before initializeCard() call)
const personaMode = ref<'rational' | 'idol' | 'hybrid' | 'strict'>('hybrid')
const personaSoul = ref('')
const personaIdentity = ref('')
const personaAddressingTitle = ref('主人')
const personaAddressingCustomName = ref('')
const personaAddressingUseCustomFirst = ref(true)
...
const personaModeOptions = computed<{ label: string, value: 'rational' | 'idol' | 'hybrid' | 'strict' }[]>(() => [
  ...
  { label: t('settings.pages.card.creation.persona-tab.modes.strict'), value: 'strict' },
])
...
<template>
  <DialogRoot :open="modelValue" @update:open="emit('update:modelValue', $event)">
    <DialogPortal>
      ...
      <DialogContent class="fixed left-1/2 top-1/2 z-[9999] m-0 max-h-[90vh] max-w-6xl w-[92vw] flex flex-col overflow-auto rounded-2xl bg-white/90 dark:bg-neutral-900/90 backdrop-blur-2xl border border-black/5 dark:border-white/5 p-6 shadow-xl outline-none 2xl:w-[60vw] lg:w-[80vw] md:w-[85vw] xl:w-[70vw] -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow sm:p-6" @interact-outside.prevent>
        <div class="w-full flex flex-col gap-5">
          ...
          <!-- Dialog tabs -->
          <div class="mt-4">
            <div class="border-b border-black/[0.06] dark:border-white/[0.06]">
          ...
          <!-- Persona -->
          <div v-else-if="activeTab === 'persona'" class="tab-content ml-auto mr-auto w-95%">
            <p class="mb-3 text-neutral-500 dark:text-neutral-400">
              {{ t('settings.pages.card.creation.persona-tab.description') }}
            </p>
            ...
            <div class="input-list ml-auto mr-auto w-90% flex flex-row flex-wrap justify-center gap-8">
              ...
              <div class="w-full">
                <label class="mb-2 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  {{ t('settings.pages.card.creation.persona-tab.mode-title') }}
                </label>
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <button
                    v-for="opt in personaModeOptions"
                    :key="opt.value"
                    type="button"
                    class="rounded-lg border p-3 text-left text-sm transition-all"
                    :class="personaMode === opt.value
                      ? 'border-primary-500 bg-primary-50 text-primary-700 dark:border-primary-400 dark:bg-primary-900/30 dark:text-primary-300'
                      : 'border-neutral-200 bg-white text-neutral-600 hover:border-neutral-300 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-300'"
                    @click="personaMode = opt.value"
                  >
                    {{ opt.label }}
                  </button>
                </div>
              ...
              <FieldInput
                v-model="personaSoul"
                :label="t('settings.pages.card.creation.persona-tab.soul-label')"
                :single-line="false"
                :description="t('settings.pages.card.creation.persona-tab.soul-description')"
              />

              <!-- Identity -->
              <FieldInput
                v-model="personaIdentity"
                :label="t('settings.pages.card.creation.persona-tab.identity-label')"
                :single-line="false"
                :description="t('settings.pages.card.creation.persona-tab.identity-description')"
              />

              <!-- Addressing -->
              <div class="w-full">
                <label class="mb-2 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                  {{ t('settings.pages.card.creation.persona-tab.addressing-title') }}
                </label>
                <div class="flex flex-row flex-wrap gap-4">
                  <FieldInput
                    v-model="personaAddressingTitle"
                    :label="t('settings.pages.card.creation.persona-tab.default-addressing')"
                    :description="t('settings.pages.card.creation.persona-tab.default-addressing-description')"
                  />
                  <FieldInput
                    v-model="personaAddressingCustomName"
                    :label="t('settings.pages.card.creation.persona-tab.custom-name')"
                    :description="t('settings.pages.card.creation.persona-tab.custom-name-description')"
                  />
                </div>
              </div>
            </div>
