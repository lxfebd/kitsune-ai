...
import type { AiriCard } from '@kitsune/stage-ui/stores/modules/persona'
...
import { Button, Select } from '@kitsune/ui'
...
const moduleSettings = computed(() => {
  if (!selectedCard.value || !selectedCard.value.extensions?.kitsune?.modules) {
    return {
  ...
  const kitsuneExt = selectedCard.value.extensions.kitsune.modules
  return {
    consciousnessProvider: kitsuneExt.consciousness?.provider || '',
    consciousness: kitsuneExt.consciousness?.model || '',
    visionProvider: kitsuneExt.vision?.provider || '',
    vision: kitsuneExt.vision?.model || '',
    speechProvider: kitsuneExt.speech?.provider || '',
    speech: kitsuneExt.speech?.model || '',
    voice: kitsuneExt.speech?.voice_id || '',
  }
})
...
const activeBackgroundId = computed({
  get: () => selectedCard.value?.extensions?.kitsune?.modules?.activeBackgroundId || 'none',
  set: async (val: string) => {
    ...
    extension.kitsune.modules.activeBackgroundId = val
...
const tabs = computed<Tab[]>(() => {
  ...
  availableTabs.push({
    id: 'gallery',
    label: t('settings.pages.card.gallery'),
            ...
            <div v-if="activeTab === 'gallery'">
              ...
              >
                <div class="flex flex-row items-center gap-3">
                  <div class="flex flex-col gap-1">
                    <h3 text-sm font-medium>
                      {{ t('settings.pages.card.pinned-background') }}
                    </h3>
                    <p text-xs text-neutral-500>
                      {{ t('settings.pages.card.pinned-background-description') }}
                    </p>
                  </div>
                  <button
                    :class="[
                      'flex items-center justify-center size-7 rounded-md',
                      'bg-white/60 dark:bg-white/[0.04] text-neutral-500',
                    ...
                    :title="t('settings.pages.card.refresh-gallery')"
                ...
                <div w-64>
                  <Select
                    ...
                    :placeholder="t('settings.pages.card.select-background')"
                  />
                </div>
              </div>
              ...
              >
                ...
                <p class="text-neutral-500 dark:text-neutral-400">
                  {{ t('settings.pages.card.no-images') }}
                </p>
              </div>
