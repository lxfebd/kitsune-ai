...
import { streamWebSpeechAPITranscription } from '@kitsune/stage-ui/stores/providers/web-speech-api'
import { useSettingsAudioDevice } from '@kitsune/stage-ui/stores/settings'
import { Button, FieldCombobox } from '@kitsune/ui'
      ...
      <ProviderSettingsContainer class="w-full md:w-[40%] space-y-6">
        ...
        >
          <div class="space-y-4">
            ...
            <FieldCombobox
              v-model="language"
              :label="t('settings.pages.providers.provider.browser-web-speech-api.recognition-language')"
              :description="t('settings.pages.providers.provider.browser-web-speech-api.recognition-language-description')"
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2">
                ...
                >
                <span class="text-sm font-medium">{{ t('settings.pages.providers.provider.browser-web-speech-api.continuous-recognition') }}</span>
              </label>
              <p class="text-xs text-neutral-500 dark:text-neutral-400">
                {{ t('settings.pages.providers.provider.browser-web-speech-api.continuous-recognition-description') }}
              </p>
            </div>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2">
                <input
                  ...
                  class="border-neutral-300 rounded text-primary-600 dark:border-neutral-700 dark:bg-neutral-900 focus:ring-primary-500"
                >
                <span class="text-sm font-medium">{{ t('settings.pages.providers.provider.browser-web-speech-api.show-interim-results') }}</span>
              </label>
              <p class="text-xs text-neutral-500 dark:text-neutral-400">
                {{ t('settings.pages.providers.provider.browser-web-speech-api.show-interim-results-description') }}
              </p>
            </div>
          </div>
        </ProviderBasicSettings>
        ...
        <div w-full rounded-xl bg="neutral-50 dark:[rgba(0,0,0,0.3)]" p-4 flex="~ col gap-4">
          ...
          <div v-if="!isWebSpeechAPIAvailable" class="border border-amber-200 rounded-lg bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-900/20">
            <div class="flex items-center gap-2 text-amber-700 dark:text-amber-400">
              ...
              <span class="text-sm font-medium">{{ t('settings.pages.providers.provider.browser-web-speech-api.not-available') }}</span>
            </div>
          </div>
          ...
          <div v-else class="flex flex-col gap-4">
            ...
            <div class="flex items-center gap-2">
              <FieldCombobox
                ...
                :label="t('settings.hearing.audio-input-device')"
                :description="t('settings.pages.providers.provider.browser-web-speech-api.audio-input-description')"
                :options="audioInputs.map(input => ({
                  label: input.label || input.deviceId,
                  value: input.deviceId,
                }))"
                :placeholder="t('settings.hearing.select-audio-input-device')"
                layout="vertical"
                class="flex-1"
              />
            </div>
            ...
            <div v-if="!selectedAudioInput" class="border border-amber-200 rounded-lg bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-900/20">
              <div class="flex items-center gap-2 text-amber-700 dark:text-amber-400">
                <div i-solar:warning-circle-line-duotone class="text-lg" />
                <span class="text-sm font-medium">{{ t('settings.hearing.select-audio-input-device') }}</span>
              </div>
            </div>
            ...
            <ErrorContainer v-if="testTranscriptionError" :title="t('settings.hearing.transcription-error')" :error="testTranscriptionError" />
            ...
            <div class="space-y-3">
              ...
              <div class="text-xs text-neutral-500 dark:text-neutral-400">
                <div>{{ t('settings.hearing.provider') }}: <span class="font-medium">Web Speech API</span></div>
