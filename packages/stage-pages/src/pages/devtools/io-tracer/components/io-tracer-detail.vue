...
import type { IOSpan, IOTurn } from '@kitsune/stage-shared'
...
<template>
    ...
    <div :class="['px-3 py-2', 'text-xs', 'flex flex-col gap-3']">
      ...
      <div>
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.timing') }}
        </div>
        <div :class="['grid grid-cols-[auto_1fr] gap-x-3 gap-y-1']">
          <template v-if="duration !== null">
            <span :class="['text-neutral-400']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.duration') }}</span>
            <span :class="['font-mono font-medium']">{{ fmtMs(duration) }}</span>
          </template>
          <template v-else>
            <span :class="['text-neutral-400']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.status') }}</span>
            <span :class="['text-amber-500 font-medium']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.in-progress') }}</span>
          </template>
          <span :class="['text-neutral-400']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.start') }}</span>
          <span :class="['font-mono']">+{{ fmtMs(relativeStart) }}</span>
          <template v-if="relativeEnd !== null">
            <span :class="['text-neutral-400']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.end') }}</span>
      ...
      <div v-if="props.span.meta.text">
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.text') }}
        </div>
        <div
          :class="[
            'p-2 rounded',
        ...
        >
          ...
          <button
            ...
            :title="t('tamagotchi.settings.devtools.pages.io-tracer.copy-text')"
      ...
      <div v-if="eventEntries.length > 0">
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.events') }}
        </div>
          ...
          >
            ...
            <div :class="['flex items-start gap-1']">
              ...
              <button
                ...
                :title="t('tamagotchi.settings.devtools.pages.io-tracer.copy-value')"
      ...
      <div v-if="relatedSpans.length > 0">
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.related-spans') }}
        </div>
      ...
      <div>
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.identity') }}
        </div>
        <div :class="['grid grid-cols-[auto_1fr] gap-x-3 gap-y-1']">
          <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.trace') }}</span>
          <span :class="['font-mono text-2.5']">{{ props.span.traceId.slice(0, 16) }}…</span>
          <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.span') }}</span>
          <span :class="['font-mono text-2.5']">{{ props.span.id.slice(0, 16) }}</span>
          <template v-if="props.span.parentSpanId">
            <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.parent') }}</span>
            <span :class="['font-mono text-2.5']">{{ props.span.parentSpanId.slice(0, 16) }}</span>
          </template>
          <template v-if="props.span.ttsCorrelationId">
            <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.segment') }}</span>
      ...
      <div>
        <div :class="['text-neutral-500 font-medium mb-1.5 uppercase tracking-wider text-2.5']">
          {{ t('tamagotchi.settings.devtools.pages.io-tracer.turn') }}
        </div>
        <div :class="['grid grid-cols-[auto_1fr] gap-x-3 gap-y-1']">
          <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.spans') }}</span>
          <span :class="['text-2.5']">{{ props.turn.spans.length }}</span>
          <template v-if="props.turn.endTs">
            <span :class="['text-neutral-400 text-2.5']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.total') }}</span>
  ...
  >
    ...
    <span :class="['text-xs']">{{ t('tamagotchi.settings.devtools.pages.io-tracer.click-span-to-inspect') }}</span>
  </div>
</template>
