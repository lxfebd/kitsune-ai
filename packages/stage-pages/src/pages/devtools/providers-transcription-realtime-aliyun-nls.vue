...
import type { ServerEvent, ServerEvents } from '@kitsune/stage-ui/stores/providers/aliyun'

import vadWorkletUrl from '@kitsune/stage-ui/workers/vad/process.worklet?worker&url'

import { errorMessageFromValue } from '@kitsune/stage-shared'
import { createAliyunNLSProvider, streamAliyunTranscription } from '@kitsune/stage-ui/stores/providers/aliyun/stream-transcription'
import { Button, FieldCombobox, FieldInput } from '@kitsune/ui'
  ...
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-semibold">
        {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.title') }}
      </h1>
      <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
        {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.description') }}
      </p>
    </div>
    ...
    <section class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2">
        <FieldInput
          ...
          :label="t('tamagotchi.settings.devtools.pages.aliyun-nls.access-key-id')"
          :description="t('tamagotchi.settings.devtools.pages.aliyun-nls.access-key-id-description')"
          placeholder="LTAI..."
        />

        <FieldInput
          v-model="credentials.accessKeySecret"
          :label="t('tamagotchi.settings.devtools.pages.aliyun-nls.access-key-secret')"
          :description="t('tamagotchi.settings.devtools.pages.aliyun-nls.access-key-secret-description')"
          placeholder="****************"
          type="password"
        />
        ...
        <FieldInput
          ...
          :label="t('tamagotchi.settings.devtools.pages.aliyun-nls.app-key')"
          :description="t('tamagotchi.settings.devtools.pages.aliyun-nls.app-key-description')"
          :placeholder="t('settings.pages.providers.provider.aliyun-nls.app-key-placeholder')"
        />

        <FieldCombobox
          v-model="credentials.region"
          :label="t('tamagotchi.settings.devtools.pages.aliyun-nls.region')"
          :description="t('tamagotchi.settings.devtools.pages.aliyun-nls.region-description')"
      ...
      <div class="flex flex-wrap items-center gap-4">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          >
            {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.start-recording') }}
          </Button>
          ...
          <Button
            ...
            @click="stopRecording"
          >
            {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.stop-recording') }}
          </Button>
          ...
          >
            {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.abort-transcription') }}
          </Button>
        </div>
      </div>
    </section>
    ...
    <section class="space-y-3">
      <h2 class="text-lg font-semibold">
        {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.transcripts') }}
      </h2>
      <div class="border border-neutral-200/80 rounded bg-neutral-50/60 p-4 text-sm dark:border-neutral-700 dark:bg-neutral-900/50">
        <div v-if="currentPartial" class="mb-3 text-neutral-500 dark:text-neutral-400">
          <div class="text-xs text-neutral-400 tracking-wide uppercase dark:text-neutral-500">
            {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.partial') }}
          </div>
        ...
        <div v-if="!transcripts.length && !currentPartial" class="text-neutral-400 dark:text-neutral-600">
          {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.waiting-for-server') }}
        </div>
        <ul class="space-y-2">
          <li
          ...
          >
            ...
            <div>
              ...
              <div v-if="!sentence.final" class="text-xs text-neutral-400">
                {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.waiting-for-final') }}
              </div>
            </div>
          </li>
        </ul>
    ...
    <section class="space-y-3">
      <h2 class="text-lg font-semibold">
        {{ t('tamagotchi.settings.devtools.pages.aliyun-nls.logs') }}
      </h2>
      <div
        ref="logsContainer"
