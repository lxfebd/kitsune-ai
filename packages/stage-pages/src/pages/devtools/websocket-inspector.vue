...
import { useWebSocketInspectorStore } from '@kitsune/stage-ui/stores/devtools/websocket-inspector'
import { Button, FieldCheckbox, Input } from '@kitsune/ui'
  ...
  <div class="h-full flex flex-col gap-4 overflow-hidden p-4">
    ...
    <div class="flex flex-col gap-4 rounded-xl bg-neutral-50 p-4 dark:bg-[rgba(0,0,0,0.3)]">
      <div class="flex items-center gap-2">
        <FieldCheckbox v-model="showIncoming" :label="t('tamagotchi.settings.devtools.pages.websocket-inspector.incoming')" />
        <FieldCheckbox v-model="showOutgoing" :label="t('tamagotchi.settings.devtools.pages.websocket-inspector.outgoing')" />
        <FieldCheckbox v-model="showHeartbeats" :label="t('tamagotchi.settings.devtools.pages.websocket-inspector.heartbeats')" />
      </div>
      ...
      <div class="flex gap-2">
        ...
        />
        <Button
          :label="t('tamagotchi.settings.devtools.pages.websocket-inspector.clear')"
        ...
        >
          ...
          <details class="group mt-2" :open="showingDetails === item.id">
            <summary class="cursor-pointer select-none text-sm font-medium" @click="showingDetails = showingDetails === item.id ? '' : item.id">
              {{ t('tamagotchi.settings.devtools.pages.websocket-inspector.payload') }}
            </summary>
            <pre
