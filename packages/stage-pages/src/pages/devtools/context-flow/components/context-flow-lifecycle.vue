...
import type { ContextLifecycleRecord } from '@kitsune/stage-ui/stores/devtools/context-observability'
...
<template>
  <Section
    :title="t('tamagotchi.settings.devtools.pages.context-flow.context-lifecycle')"
    icon="i-solar:history-bold-duotone"
    inner-class="gap-3"
    :expand="false"
  >
    <div :class="['text-xs', 'text-neutral-500', 'dark:text-neutral-400']">
      {{ filteredRecords.length }} {{ t('tamagotchi.settings.devtools.pages.context-flow.lifecycle-record-count') }}
    </div>

    <div v-if="!filteredRecords.length" :class="['text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
      {{ t('tamagotchi.settings.devtools.pages.context-flow.no-lifecycle-record') }}
    </div>
