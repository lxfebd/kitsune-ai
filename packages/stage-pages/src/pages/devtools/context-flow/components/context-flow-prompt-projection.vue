...
import type { PromptProjectionSnapshot } from '@kitsune/stage-ui/stores/devtools/context-observability'
...
<template>
  <Section
    :title="t('tamagotchi.settings.devtools.pages.context-flow.next-prompt-projection')"
  ...
  >
    ...
    >
      <div :class="['text-xs', 'font-medium', 'uppercase', 'tracking-[0.08em]', currentPromptText ? 'text-emerald-600 dark:text-emerald-400' : 'text-neutral-400 dark:text-neutral-500']">
        {{ t('tamagotchi.settings.devtools.pages.context-flow.prompt-readiness') }}
      </div>
      ...
      <div v-else :class="['mt-2', 'text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
        {{ t('tamagotchi.settings.devtools.pages.context-flow.no-retained-ready') }}
      </div>
    </div>
    ...
    <div :class="['grid', 'gap-3', 'xl:grid-cols-2']">
      ...
      >
        <div :class="['text-xs', 'font-medium', 'uppercase', 'tracking-[0.08em]', 'text-neutral-400', 'dark:text-neutral-500']">
          {{ t('tamagotchi.settings.devtools.pages.context-flow.current-derived-prompt-block') }}
        </div>
      ...
      >
        <div :class="['flex', 'flex-wrap', 'items-center', 'justify-between', 'gap-2']">
          <div :class="['text-xs', 'font-medium', 'uppercase', 'tracking-[0.08em]', 'text-neutral-400', 'dark:text-neutral-500']">
            {{ t('tamagotchi.settings.devtools.pages.context-flow.last-actual-compose') }}
          </div>
        ...
        <template v-if="lastProjection">
          ...
          <div v-else :class="['mt-3', 'text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
            {{ t('tamagotchi.settings.devtools.pages.context-flow.last-compose-no-context') }}
          </div>

          <details :class="['mt-3']">
            <summary :class="['cursor-pointer', 'text-xs', 'text-neutral-500', 'dark:text-neutral-400']">
              {{ t('tamagotchi.settings.devtools.pages.context-flow.details') }}
            </summary>
        ...
        <div v-else-if="currentPromptText" :class="['mt-3', 'text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
          {{ t('tamagotchi.settings.devtools.pages.context-flow.retained-context-ready') }}
        </div>

        <div v-else :class="['mt-3', 'text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
          {{ t('tamagotchi.settings.devtools.pages.context-flow.no-compose-snapshot') }}
        </div>
      </div>
    </div>
  </Section>
