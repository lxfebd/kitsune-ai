...
import type { ContextBucketSnapshot } from '@kitsune/stage-ui/stores/chat/context-store'
...
<template>
  <Section
    :title="t('tamagotchi.settings.devtools.pages.context-flow.active-context-state')"
  ...
  >
    ...
    <div v-if="!filteredBuckets.length" :class="['text-sm', 'text-neutral-500', 'dark:text-neutral-400']">
      {{ t('tamagotchi.settings.devtools.pages.context-flow.no-retained-contexts') }}
    </div>
