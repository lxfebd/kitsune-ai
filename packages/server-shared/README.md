# @kitsune/server-shared

The shared type definitions for all server-side packages of Project Kitsune.

## Usage

```shell
ni @kitsune/server-shared -D # from @antfu/ni, can be installed via `npm i -g @antfu/ni`
pnpm i @kitsune/server-shared -D
yarn i @kitsune/server-shared -D
npm i @kitsune/server-shared -D
```

```typescript
import type { WebSocketEvents } from '@kitsune/server-shared'
```
