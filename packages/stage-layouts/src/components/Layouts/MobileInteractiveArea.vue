...
import { useThreeViewControl } from '@kitsune/stage-ui-three'
import { ChatHistory, HearingConfigDialog } from '@kitsune/stage-ui/components'
import { ChatSessionsDrawer } from '@kitsune/stage-ui/components/scenarios/chat'
import { useAnalytics, useAudioAnalyzer, useRouterStatus } from '@kitsune/stage-ui/composables'
...
import { useL2dViewControl } from '@kitsune/stage-ui/stores/live2d'
import { useLlmRouter } from '@kitsune/stage-ui/stores/llm-router'
...
import { BasicTextarea, useTheme } from '@kitsune/ui'
...
async function handleSend() {
  ...
  try {
    // Route the message to the best provider
    const routeDecision = llmRouter.resolve(textToSend, activeProvider.value, activeModel.value)
    const targetProviderId = routeDecision.providerId
    const targetModel = routeDecision.model || activeModel.value

    const providerConfig = providersStore.getProviderConfig(targetProviderId)

    await ingest(textToSend, {
      chatProvider: await providersStore.getProviderInstance(targetProviderId) as ChatProvider,
      model: targetModel,
      providerConfig,
    })
  }
    ...
    <div relative w-full self-end>
      ...
      <div bg="white dark:neutral-800" max-h-100dvh max-w-100dvw w-full flex gap-1 overflow-auto px-3 pt-2 :style="{ paddingBottom: `${Math.max(Number.parseFloat(screenSafeArea.bottom.value.replace('px', '')), 12)}px` }">
        ...
        <button
          ...
          :title="t('stage.stop-speaking')"
          :aria-label="t('stage.stop-speaking')"
