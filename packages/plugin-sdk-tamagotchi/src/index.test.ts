import type {
  ...
  KitRef,
  KitUseResult,
} from '@kitsune/plugin-sdk'
import type { HostDataRecord } from '@kitsune/plugin-sdk/plugin-host'

import type { ToolKitRuntime } from './tools'

import { DisposableStore } from '@kitsune/plugin-sdk'
...
describe('plugin-sdk-tamagotchi', () => {
  ...
  it('exposes toolKit as a module-scoped kit client without a gamelet runtime', async () => {
    ...
    await client.registerToolsetPrompt({
      id: 'chess-toolset',
      prompt: {
        id: 'kitsune-plugin-game-chess.prompt',
  ...
  /**
   * @example
   * expect(registerToolsetPrompt).toHaveBeenCalledWith({ id: 'kitsune-plugin-game-chess.prompt', prompt: expect.any(Object) })
