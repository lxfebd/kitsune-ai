...
import { listenBeatSyncBeatSignal } from '@kitsune/stage-shared/beat-sync'
import { loadModelProfile, motionStylePresets, type ModelProfile } from '@soullink-emotion/engine'
...
let soullinkBridge: ReturnType<typeof useSoullinkBridge> | null = null
...
async function loadModel() {
  ...
  try {
    ...
    // NOTE: AutoEyeBlink is skipped when Soullink is active — Soullink's IdleEngine
    // handles blinking with stateful double-blink, gaze, breathing, and body sway.
    // Registering both would cause ParamEyeLOpen/ParamEyeROpen write conflicts.
    motionManagerUpdate.register(useMotionUpdatePluginLipSync(mouthOpenSize, nowSpeaking), 'final')

    // Soullink Emotion Engine integration
    // Load the ModelProfile from the model's asset directory (if available)
    // Falls back to a heuristic profile based on standard Cubism 4 parameter IDs
    try {
      let profile: ModelProfile | null = null
      const profileUrl = modelSrcRef.value?.replace(/[^/\\]+$/, 'soullink.profile.json')
      if (profileUrl) {
        try {
          const result = await loadModelProfile(profileUrl)
          profile = result.profile
        }
        catch {
          // Profile file not found — create a default heuristic profile
        }
      }
      if (!profile) {
        // Default profile using standard Cubism 4 parameter IDs
        profile = {
          modelId: props.modelId ?? 'default',
          displayName: props.modelId ?? 'default',
          version: '1.0.0',
          modelPath: modelSrcRef.value ?? '',
          parameterMap: {
        ...
        console.info('[Live2D] Using default heuristic profile for model:', profile.modelId)
      }
      soullinkBridge = useSoullinkBridge({ profile, motionStyle: motionStylePresets.natural })
      // Inject common Cubism decoration parameters for private VAD overlay
      // (blush, tear, sweat, anger marks, etc.)
      soullinkBridge.setPrivateVADParameters({
        ParamCheek: { min: 0, max: 1, default: 0 },
        ParamTere: { min: 0, max: 1, default: 0 },
        ParamTear: { min: 0, max: 1, default: 0 },
        ParamSweat: { min: 0, max: 1, default: 0 },
        ParamAnger: { min: 0, max: 1, default: 0 },
        ParamHeart: { min: 0, max: 1, default: 0 },
        ParamShadow: { min: 0, max: 1, default: 0 },
      })
      // Register bridge plugin in final stage — writes Soullink parameters on top of all other plugins
      motionManagerUpdate.register(soullinkBridge.createBridgePlugin(), 'final')
      console.info('[Live2D] Soullink Emotion Engine initialized for model:', profile.modelId)
    }
    catch (error) {
      // Soullink failed — fall back to legacy AutoEyeBlink plugin for blinking
      console.debug('[Live2D] Soullink Emotion Engine init failed, using standard emotion pipeline', error)
    ...
    // NOTICE:
    // pixi-live2d-display v0.5.0-beta removed `motionStart`/`motionFinish` events
    // from MotionManager. We wrap `startMotion` to detect motion start and use
    // the `state.complete()` path to detect finish via a playing-state watcher.
    const originalStartMotion = motionManager.startMotion.bind(motionManager)
    motionManager.startMotion = async (group: string, index: number, priority?: any) => {
      localCurrentMotion.value = { group, index }
      return originalStartMotion(group, index, priority)
    }

    // Watch for motion finish by detecting when playing transitions to false
    let wasPlaying = motionManager.playing
    const checkMotionFinish = () => {
      if (wasPlaying && !motionManager.playing) {
        const selectedMotionGroup = localStorage.getItem('selected-runtime-motion-group')
        const selectedMotionIndex = localStorage.getItem('selected-runtime-motion-index')
      ...
      wasPlaying = motionManager.playing
    }
    motionManagerUpdate.register(() => { checkMotionFinish() }, 'post')
...
watch(modelSrcRef, async (newSrc, oldSrc) => {
  // Skip reload if the URL hasn't actually changed (e.g., same blob URL
  // re-assigned by updateStageModel during cross-tab localStorage sync).
  if (newSrc === oldSrc)
    return
  await loadModel()
}, { immediate: true })
...
// 页面隐藏时暂停 dropShadow RAF，visible 时按条件恢复，避免后台 CPU 占用
function handleDropShadowVisibilityChange() {
  if (document.hidden) {
    cancelAnimationFrame(dropShadowAnimationId.value)
    dropShadowAnimationId.value = 0
  }
  else {
    if (themeColorsHueDynamic.value && live2dShadowEnabled.value)
      dropShadowAnimationId.value = requestAnimationFrame(updateDropShadowFilterLoop)
  }
}
...
onMounted(async () => {
  updateDropShadowFilter()
  document.addEventListener('visibilitychange', handleDropShadowVisibilityChange)
})

onUnmounted(() => {
  isUnmounted = true
  document.removeEventListener('visibilitychange', handleDropShadowVisibilityChange)
  resizeAnimation?.pause()
  cancelAnimationFrame(dropShadowAnimationId.value)
  dropShadowAnimationId.value = 0
  ...
  // Clean up model resources to prevent WebGL texture leaks
  if (model.value) {
    try {
      model.value.destroy({ children: true, texture: true, baseTexture: true })
    }
...
defineExpose({
  ...
  initialModelHeight,
  initialModelWidth,
  get soullinkBridge() { return soullinkBridge },
})
