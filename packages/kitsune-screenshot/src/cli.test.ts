...
describe('parseAiriScreenshotCliArguments', () => {
  ...
  it('uses a target-specific default output directory', () => {
    expect(parseAiriScreenshotCliArguments([
      ...
      target: 'tamagotchi',
      scenario: 'settings-connection',
      outputDir: '.vishot/kitsune-screenshot/tamagotchi',
      format: 'png',
    })
  })
  ...
  it('rejects unsupported targets', () => {
    expect(() => parseAiriScreenshotCliArguments([
      ...
      '--route',
      '/settings/characters',
    ])).toThrow('Unsupported Kitsune screenshot target "stage-web". Expected "tamagotchi".')
  })
  ...
  it('rejects missing tamagotchi scenario input', () => {
    expect(() => parseAiriScreenshotCliArguments([
      'capture',
      'tamagotchi',
    ])).toThrow('Usage: kitsune-screenshot capture tamagotchi --scenario <preset-or-path> [--output-dir <dir>]')
  })
})
...
describe('main', () => {
  ...
  it('rejects non-zero delegated runner exits', async () => {
    ...
    await expect(main([
      ...
      '--scenario',
      'packages/scenarios-stage-tamagotchi-electron/src/scenarios/demo-hearing-dialog.ts',
    ])).rejects.toThrow('Kitsune screenshot command exited with code 1.')
  })
})

describe('discoverTamagotchiScenarios', () => {
  it('discovers root scenario files and folder scenarios with index entrypoints', async () => {
    const scenariosRoot = await mkdtemp(path.join(tmpdir(), 'kitsune-screenshot-scenarios-'))
