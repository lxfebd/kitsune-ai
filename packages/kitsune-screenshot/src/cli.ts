...
const usageMessage = 'Usage: kitsune-screenshot capture tamagotchi --scenario <preset-or-path> [--output-dir <dir>]'
...
function createAiriScreenshotCli() {
  const cli = cac('kitsune-screenshot')
...
export function parseAiriScreenshotCliArguments(argv: string[]): AiriScreenshotCliArguments {
  ...
  const parsed = cli.parse(['node', 'kitsune-screenshot', ...normalizedArgv], { run: false })
  ...
  if (target !== 'tamagotchi') {
    throw new Error(`Unsupported Kitsune screenshot target "${target ?? ''}". Expected "tamagotchi".`)
  }
 ...
 *
 * Use when:
 * - Invoked through `pnpm -F @kitsune/screenshot capture ...`.
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  const output = await x('pnpm', [
    '-F',
    '@kitsune/vishot-runner-electron',
    'capture',
    runnerScenarioPath,
    '--output-dir',
    runnerOutputDir,
  ...
  if (output.exitCode !== 0) {
    throw new Error(`Kitsune screenshot command exited with code ${output.exitCode ?? 'unknown'}.`)
  }
}
