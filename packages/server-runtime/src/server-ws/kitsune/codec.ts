import type { WebSocketBaseEvent, WebSocketEvent } from '@kitsune/server-shared/types'

import { MessageHeartbeatKind } from '@kitsune/server-shared/types'
import { parse, stringify } from 'superjson'
import { check, objectWithRest, pipe, safeParse, string, unknown } from 'valibot'

const invalidKitsuneWebSocketEventFormatMessage = 'Invalid WebSocket event format.'
...
/** Error thrown when parsed websocket text is not a Kitsune event envelope. */
export class InvalidEventError extends Error {
  readonly source?: unknown

  constructor(options: InvalidEventErrorOptions = {}) {
    super(invalidKitsuneWebSocketEventFormatMessage, { cause: options.cause })
    this.name = 'InvalidEventError'
    this.source = options.source
  }
}

/** Checks whether an error came from Kitsune websocket event envelope validation. */
...
/** Parses one Kitsune websocket protocol event from SuperJSON or plain JSON text. */
export function parseEvent(text: string): WebSocketEvent {
  ...
  // Keep this until all Kitsune websocket clients share one non-wrapper wire format.
...
/** Serializes one Kitsune websocket protocol event with the existing SuperJSON wire format. */
