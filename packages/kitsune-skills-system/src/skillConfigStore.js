...
class SkillConfigStore {
  ...
  async ensureExistsAsync() {
    try {
      await fsp.access(this.configPath);
      return;
    } catch {
      await fsp.mkdir(path.dirname(this.configPath), { recursive: true });
      await fsp.writeFile(this.configPath, YAML.stringify(DEFAULT_SKILLS_CONFIG_CONTENT), 'utf8');
    }
  }

  async loadRawYaml() {
    await this.ensureExistsAsync();
    return fsp.readFile(this.configPath, 'utf8');
  }

  async saveRawYaml(rawYaml) {
    if (typeof rawYaml !== 'string') throw new Error('rawYaml must be a string');
    const parsed = YAML.parse(rawYaml);
    normalizeSkillsConfig(parsed); // 校验
    await fsp.writeFile(this.configPath, rawYaml, 'utf8');
  }

  async load() {
    const raw = await this.loadRawYaml();
