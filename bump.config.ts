...
const androidVersionCodePattern = /^KITSUNE_VERSION_CODE=(\d+)$/m
...
async function syncAndroidVersion(version: string) {
  ...
  if (!currentVersionCodeMatch) {
    throw new TypeError('Android version file does not contain a valid KITSUNE_VERSION_CODE')
  }
  ...
  if (!Number.isSafeInteger(currentVersionCode) || currentVersionCode < 1) {
    throw new TypeError(`Android KITSUNE_VERSION_CODE is invalid: ${currentVersionCodeMatch[1]}`)
  }

  const nextVersionCode = currentVersionCode + 1
  const nextAndroidVersionContent = `KITSUNE_VERSION_NAME=${version}\nKITSUNE_VERSION_CODE=${nextVersionCode}\n`
