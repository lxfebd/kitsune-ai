  ...
  outputs =
    ...
    in
    {
      ...
      packages = forAllSystems (
        system:
        { default = self.packages.${system}.kitsune; } // self.overlays.kitsune (pkgsForSystem system) null
      );

      overlays = {
        default = self.overlays.kitsune;
        kitsune = final: _: {
      ...
      devShells = forAllSystems (
        ...
        {
          default = mkShell {
            inputsFrom = [ self.packages.${system}.kitsune ];
            packages = [
              nixd
              nixfmt
              nixfmt-tree
