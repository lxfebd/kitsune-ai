...
[airi-server-overview-cloud.json](../../otel/grafana/dashboards/airi-server-overview-cloud.json) 由 [`build.ts`](../../otel/grafana/dashboards/build.ts) 生成（**直接改 JSON 会在下次 regenerate 时被覆盖；改 build.ts**），跑 `pnpm -F @kitsune/server otel:dashboards` 重新生成。从上到下：
...
```sh
pnpm -F @kitsune/server exec node --import tsx ./src/scripts/otel/smoke.ts
```
...
6. 跑 `pnpm -F @kitsune/server exec node --import tsx ./src/scripts/otel/smoke.ts` 确认注册
7. 更新本文档对应章节
