...
```sh
pnpm -F @kitsune/server dev                # dev with dotenvx (.env.local)
pnpm -F @kitsune/server typecheck
pnpm -F @kitsune/server exec vitest run    # all server tests
pnpm exec vitest run apps/server/src/...     # single test file
pnpm -F @kitsune/server db:generate        # drizzle-kit generate
pnpm -F @kitsune/server db:push            # drizzle-kit push
pnpm -F @kitsune/server auth:generate      # better-auth → src/schemas/accounts.ts
```
...
- **Schemas** (`src/schemas/`): Drizzle table definitions. Migrations in `@kitsune/server-schema`.
