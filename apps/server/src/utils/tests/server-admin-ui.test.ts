...
describe('admin UI URL helpers', () => {
  ...
  it('routes server-dev default admin UI redirects to the matching Pages branch', () => {
    expect(buildAdminUiRedirectUrl(
      ...
      'https://kitsune-server-dev.up.railway.app/admin/users?query=alice',
      'https://kitsune-server-dev.up.railway.app',
    )).toBe(
      'https://server-dev.kitsune-server-admin.pages.dev/users?query=alice&api_server_url=https%3A%2F%2Fkitsune-server-dev.up.railway.app',
    )
  })
