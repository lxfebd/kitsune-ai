...
describe('origin utils', () => {
  ...
  it('allows the standalone auth and admin UI origins', () => {
    expect(getTrustedOrigin('https://accounts.kitsune.build')).toBe('https://accounts.kitsune.build')
    expect(getTrustedOrigin('https://server-dev.kitsune-server-auth.pages.dev')).toBe('https://server-dev.kitsune-server-auth.pages.dev')
    expect(getTrustedOrigin('https://admin.kitsune.build')).toBe('https://admin.kitsune.build')
    expect(getTrustedOrigin('https://server-dev.kitsune-server-admin.pages.dev')).toBe('https://server-dev.kitsune-server-admin.pages.dev')
  })
  ...
  it('prefers a trusted referer origin', () => {
    const request = new Request('http://localhost/api/v1/stripe/checkout', {
      headers: {
        referer: 'https://kitsune.moeru.ai/settings/flux',
        origin: 'https://example.com',
      },
    })

    expect(resolveTrustedRequestOrigin(request)).toBe('https://kitsune.moeru.ai')
  })
  ...
  it('collects api and request origins for auth', () => {
    ...
    expect(getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.kitsune.moeru.ai',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    }, request)).toEqual([
      'https://api.kitsune.moeru.ai',
      'https://kitsune.moeru.ai',
      'https://accounts.kitsune.build',
      'https://server-dev.kitsune-server-auth.pages.dev',
      'https://admin.kitsune.build',
      'https://server-dev.kitsune-server-admin.pages.dev',
      'http://localhost:*',
      'http://127.0.0.1:*',
      'http://localhost:5173',
    ])
  ...
  describe('resolveCheckoutRedirectBase', () => {
    const fallback = 'https://kitsune.moeru.ai'
  ...
  it('does not include native deep-link schemes in Better Auth trustedOrigins', () => {
    ...
    expect(getTrustedOrigin('ai.kitsune.pocket://links')).toBe('ai.kitsune.pocket://links')

    const authOrigins = getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.kitsune.build',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    })

    expect(authOrigins).not.toContain('capacitor://localhost')
    expect(authOrigins).not.toContain('ai.kitsune.pocket://links')
  })
  ...
  // UI sends callbackURL=https://accounts.kitsune.build/ui/verify-email?verified=true,
  ...
  it('includes built-in first-party origins for email verification callbacks without request headers', () => {
    expect(getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.kitsune.build',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    })).toContain('https://accounts.kitsune.build')
  })
})
