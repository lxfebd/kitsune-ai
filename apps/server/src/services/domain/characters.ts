...
export function createCharacterService(db: Database, metrics?: EngagementMetrics | null) {
  return {
    ...
    async update(id: string, data: Partial<schema.NewCharacter> & {
      ...
      // TODO(audit): Return a stable single-object response shape for HTTP callers — Drizzle returning() arrays across the service boundary make route contracts drift
      const { prompts, ...fields } = data
      const result = await db.transaction(async (tx) => {
        const updated = await tx.update(schema.character)
        ...
        // Replace prompts wholesale: delete all existing rows for the character,
        // then insert the new set. Makes PATCH idempotent for the
        // system/personality/greetings set regardless of which ones changed.
        if (prompts) {
          await tx.delete(schema.characterPrompts)
            .where(eq(schema.characterPrompts.characterId, id))
          if (prompts.length) {
            await tx.insert(schema.characterPrompts).values(
              prompts.map(p => ({ ...p, characterId: id })),
            )
          }
        }
