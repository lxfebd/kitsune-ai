...
import {
  KITSUNE_ATTR_GEN_AI_GATEWAY_FALLBACK_DEPTH,
  KITSUNE_ATTR_GEN_AI_GATEWAY_KEY_ID,
  KITSUNE_ATTR_GEN_AI_GATEWAY_UPSTREAM_INDEX,
  KITSUNE_ATTR_GEN_AI_GATEWAY_UPSTREAM_URL,
...
export function createLlmRouterService(options: CreateLlmRouterServiceOptions) {
  ...
  > {
    ...
    for (const key of rotator) {
      try {
        ...
        // NOTICE:
        // We compose three AbortSignals — per-attempt timeout, the caller's
        // signal, and the full-chain timeout — using AbortSignal.any for
        // clean composition. This ensures any signal aborting will cancel
        // the fetch request.
        const attemptCtrl = new AbortController()
        const timeoutHandle = setTimeout(() => attemptCtrl.abort(new Error('attempt-timeout')), perAttemptTimeoutMs)

        // Build list of signals to compose
        const signals: AbortSignal[] = [attemptCtrl.signal]
        if (req.abortSignal) signals.push(req.abortSignal)
        if (fullChainSignal) signals.push(fullChainSignal)
        ...
        if (response.ok) {
          ...
          trace.getActiveSpan()?.setAttributes({
            [KITSUNE_ATTR_GEN_AI_GATEWAY_UPSTREAM_URL]: upstream.baseURL,
            [KITSUNE_ATTR_GEN_AI_GATEWAY_UPSTREAM_INDEX]: upstreamIndex,
            [KITSUNE_ATTR_GEN_AI_GATEWAY_KEY_ID]: key.id,
            [KITSUNE_ATTR_GEN_AI_GATEWAY_FALLBACK_DEPTH]: attemptIndex,
          })
  ...
  async function route(req: LlmRouteRequest, ctx?: LlmRouteContext): Promise<Response> {
    ...
    // NOTICE:
    // Enforce fullChainTimeoutMs to prevent pathological multi-upstream chains
    // from blocking indefinitely. Without this, a request with 5 upstreams ×
    // 3 keys × 30s timeout could block for 450 seconds.
    const fullChainTimeoutMs = defaults.fullChainTimeoutMs ?? 60_000
    const fullChainAc = new AbortController()
    const fullChainTimer = setTimeout(() => fullChainAc.abort(), fullChainTimeoutMs)

    // Merge caller signal with full-chain signal
    const mergedSignal = req.abortSignal
      ? AbortSignal.any([req.abortSignal, fullChainAc.signal])
      : fullChainAc.signal
