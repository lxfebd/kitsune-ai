 ...
 *
 * Before:
 * - `{ fromEmail: 'noreply@a.io', fromName: 'Kitsune AI' }`
 *
 * After:
 * - `'Kitsune AI <noreply@a.io>'`
 */
...
export function createEmailService(config: EmailConfig, logger: Logger = useLogger('email'), metrics?: EmailMetrics | null): EmailService {
  ...
  return {
    ...
    async sendVerification({ to, url }) {
      await send({
        to,
        subject: 'Verify your email for Kitsune AI',
    ...
    async sendPasswordReset({ to, url }) {
      await send({
        to,
        subject: 'Reset your Kitsune AI password',
    ...
    async sendMagicLink({ to, url }) {
      await send({
        to,
        subject: 'Your Kitsune AI sign-in link',
    ...
    async sendChangeEmailConfirmation({ to, newEmail, url }) {
      await send({
        to,
        subject: 'Confirm your new email address for Kitsune AI',
    ...
    async sendDeleteAccountVerification({ to, url }) {
      await send({
        to,
        subject: 'Confirm account deletion for Kitsune AI',
...
function renderVerificationHtml(url: string): string {
  return renderActionEmailHtml({
    ...
    body: 'Welcome to Kitsune AI. Click the button below to confirm this is your email address.',
...
function renderVerificationText(url: string): string {
  return renderActionEmailText({
    heading: 'Verify your email',
    body: 'Welcome to Kitsune AI. Open this link to confirm your email address:',
    url,
    footer: 'If you did not create an account, you can safely ignore this email.',
  })
}
...
function renderPasswordResetHtml(url: string): string {
  return renderActionEmailHtml({
    ...
    body: 'We received a request to reset the password for your Kitsune AI account.',
...
function renderPasswordResetText(url: string): string {
  return renderActionEmailText({
    heading: 'Reset your password',
    body: 'Open this link to reset your Kitsune AI password:',
    url,
    footer: 'If you did not request this, you can safely ignore this email — your password will not change.',
  })
}
...
function renderChangeEmailHtml(url: string, newEmail: string): string {
  return renderActionEmailHtml({
    ...
    body: `Confirm that ${newEmail} should become your Kitsune AI account email.`,
...
function renderChangeEmailText(url: string, newEmail: string): string {
  return renderActionEmailText({
    heading: 'Confirm your new email',
    body: `Confirm that ${newEmail} should become your Kitsune AI account email by opening this link:`,
    url,
    footer: 'If you did not request this change, contact support immediately.',
  })
}
...
function renderDeleteAccountHtml(url: string): string {
  return renderActionEmailHtml({
    ...
    body: 'Click below to permanently delete your Kitsune AI account. This cannot be undone. Active subscription will be canceled, Flux balance cleared. Link expires in 24 hours.',
...
function renderDeleteAccountText(url: string): string {
  return renderActionEmailText({
    heading: 'Confirm account deletion',
    body: 'Open this link to permanently delete your Kitsune AI account. This cannot be undone. Active subscription will be canceled, Flux balance cleared. Link expires in 24 hours.',
    url,
    footer: 'Did not request this? Ignore this email and rotate your password.',
  })
}
