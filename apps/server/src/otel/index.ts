...
import {
  METRIC_KITSUNE_EMAIL_DURATION,
  METRIC_KITSUNE_EMAIL_FAILURES,
  METRIC_KITSUNE_EMAIL_SEND,
  METRIC_KITSUNE_FLUX_CREDITED,
  METRIC_KITSUNE_FLUX_UNBILLED,
  METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_INVALID_HMAC,
  METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_RELOAD,
  METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_WRITE,
  METRIC_KITSUNE_GEN_AI_GATEWAY_DECRYPT_FAILURES,
  METRIC_KITSUNE_GEN_AI_GATEWAY_FALLBACK_COUNT,
  METRIC_KITSUNE_GEN_AI_GATEWAY_KEY_EXHAUSTED_COUNT,
  METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_INFLIGHT,
  METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_SATURATION_MARKED,
  METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_SLOT_REJECTED,
  METRIC_KITSUNE_GEN_AI_GATEWAY_SAME_STATUS_EXHAUSTION,
  METRIC_KITSUNE_GEN_AI_GATEWAY_SUBSCRIBER_STATE,
  METRIC_KITSUNE_GEN_AI_GATEWAY_UPSTREAM_ERRORS,
  METRIC_KITSUNE_GEN_AI_STREAM_INTERRUPTED,
  METRIC_KITSUNE_OBSERVABILITY_READ_ERRORS,
  METRIC_KITSUNE_PRODUCT_EVENTS,
  METRIC_KITSUNE_RATE_LIMIT_BLOCKED,
  METRIC_KITSUNE_STRIPE_REVENUE,
  METRIC_KITSUNE_TTS_CHARS,
  METRIC_KITSUNE_TTS_PREFLIGHT_REJECTIONS,
...
export function initOtel(env: Env): OtelInstance | null {
  ...
  const revenue: RevenueMetrics = {
    ...
    stripeRevenue: meter.createCounter(METRIC_KITSUNE_STRIPE_REVENUE, {
    ...
    fluxCredited: meter.createCounter(METRIC_KITSUNE_FLUX_CREDITED, {
      description: 'Total flux credited to user balances, by source',
    }),
    fluxUnbilled: meter.createCounter(METRIC_KITSUNE_FLUX_UNBILLED, {
      description: 'Flux owed but unbilled (post-stream debit failed). Real revenue leak.',
    }),
    ttsChars: meter.createCounter(METRIC_KITSUNE_TTS_CHARS, {
      description: 'TTS input characters processed (billing base unit)',
    }),
    ttsPreflightRejections: meter.createCounter(METRIC_KITSUNE_TTS_PREFLIGHT_REJECTIONS, {
  ...
  const genAi: GenAiMetrics = {
    ...
    streamInterrupted: meter.createCounter(METRIC_KITSUNE_GEN_AI_STREAM_INTERRUPTED, {
  ...
  const gateway: GatewayMetrics = {
    fallbackCount: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_FALLBACK_COUNT, {
      description: 'Per-attempt fallback events in the in-process LLM/TTS router',
    }),
    upstreamErrors: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_UPSTREAM_ERRORS, {
      description: 'Upstream error responses received during fallback iteration',
    }),
    keyExhaustedCount: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_KEY_EXHAUSTED_COUNT, {
      description: 'All keys (across all upstreams) failed in a single request — primary user-facing alert',
    }),
    sameStatusExhaustion: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_SAME_STATUS_EXHAUSTION, {
      description: 'All keys in one request failed with the same upstream status (account-level rate-limit signal)',
    }),
    configReload: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_RELOAD, {
      description: 'Local in-memory router config cache reloaded (by source: pubsub / ttl / manual)',
    }),
    decryptFailures: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_DECRYPT_FAILURES, {
      description: 'Envelope-crypto decryption auth-tag failures (config corruption or rotation misstep)',
    }),
    subscriberState: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_SUBSCRIBER_STATE, {
      description: 'Pub/Sub subscriber lifecycle state transitions',
    }),
    configWrite: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_WRITE, {
      description: 'Admin endpoint LLM_ROUTER_CONFIG write events (audit-trail surrogate)',
    }),
    configInvalidHmac: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_CONFIG_INVALID_HMAC, {
      description: 'Pub/Sub invalidation messages dropped due to HMAC mismatch (forged or replayed)',
    }),
    poolSlotRejected: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_SLOT_REJECTED, {
      description: 'Capacity-aware TTS routing skipped a pool already at its app_id concurrency cap',
    }),
    poolSaturationMarked: meter.createCounter(METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_SATURATION_MARKED, {
      description: 'TTSpool circuit-broken after exhausting with a 429 (app_id concurrency exceeded)',
    }),
    poolInflight: meter.createObservableGauge(METRIC_KITSUNE_GEN_AI_GATEWAY_POOL_INFLIGHT, {
  ...
  const email: EmailMetrics = {
    send: meter.createCounter(METRIC_KITSUNE_EMAIL_SEND, {
      description: 'Transactional emails accepted by Resend',
    }),
    failures: meter.createCounter(METRIC_KITSUNE_EMAIL_FAILURES, {
      description: 'Transactional email send failures',
    }),
    duration: meter.createHistogram(METRIC_KITSUNE_EMAIL_DURATION, {
  ...
  const rateLimit: RateLimitMetrics = {
    blocked: meter.createCounter(METRIC_KITSUNE_RATE_LIMIT_BLOCKED, {
      description: 'Requests blocked by rate limiter',
    }),
  }

  const observability: ObservabilityMetrics = {
    metricReadErrors: meter.createCounter(METRIC_KITSUNE_OBSERVABILITY_READ_ERRORS, {
      description: 'Failures reading metric values inside gauge callbacks',
    }),
  }

  const product: ProductMetrics = {
    events: meter.createCounter(METRIC_KITSUNE_PRODUCT_EVENTS, {
