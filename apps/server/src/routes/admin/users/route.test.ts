...
describe('admin users — auth guards', () => {
  // NOTICE: adminGuard is disabled, so requests pass through to handler
  // Without user context, the handler returns 500 due to missing user
  it('returns 500 when unauthenticated (guard disabled, no user context)', async () => {
  ...
  it('allows request for a non-admin user (guard disabled)', async () => {
    const services = createServices()
