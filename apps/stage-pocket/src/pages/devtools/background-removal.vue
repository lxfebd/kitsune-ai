...
import { createBackgroundRemovalAdapter } from '@kitsune/stage-ui/libs/inference/adapters/background-removal'
import { Button, Checkbox, InputFile } from '@kitsune/ui'
    ...
    <template v-else>
      ...
      <div flex flex-wrap items-center justify-between gap-4>
        ...
        <div flex gap-2>
          <Button
            v-if="pendingCount > 0"
            :label="processing ? t('stage.devtools.background-removal.processing', { percent: progressPercent }) : t('stage.devtools.background-removal.process', { count: pendingCount })"
            :disabled="processing"
            :loading="processing"
          ...
          <Button
            v-if="doneCount > 0"
            variant="secondary"
            :label="t('stage.devtools.background-removal.download-all', { count: doneCount })"
          ...
          <Button
            ...
            :label="t('stage.devtools.background-removal.clear-all')"
