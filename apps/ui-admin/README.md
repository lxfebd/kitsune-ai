...
```sh
pnpm -F @kitsune/ui-admin dev
pnpm -F @kitsune/ui-admin typecheck
pnpm -F @kitsune/ui-admin build
```

## Build Output

`pnpm -F @kitsune/ui-admin build` writes to `apps/ui-admin/dist`. Server builds do not package this output; deploy the directory through the admin Cloudflare Pages workflow.
