...
import { Button, FieldInput, FieldSelect, FieldTextArea } from '@kitsune/ui'
...
<template>
  <section :class="['rounded-lg', 'border', 'border-neutral-200', 'bg-white', 'p-4', 'dark:border-neutral-800', 'dark:bg-neutral-900']">
    ...
    <div v-else-if="slice.kind === 'unspeech'" :class="['space-y-4']">
      <FieldInput v-model="slice.restBaseURL" input-class="font-mono text-xs" label="REST base URL" placeholder="http://kitsune-unspeech.railway.internal:5933" required />
      ...
      <div v-if="slice.streamingEnabled" :class="['grid', 'gap-4', 'md:grid-cols-2']">
        <FieldInput v-model="slice.streamingUpstreamURL" input-class="font-mono text-xs" label="Streaming WebSocket URL" placeholder="ws://kitsune-unspeech.railway.internal:5933/v1/audio/speech/stream" required />
