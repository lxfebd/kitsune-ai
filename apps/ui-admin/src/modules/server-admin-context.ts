...
const DEFAULT_API_SERVER_ORIGINS_BY_ADMIN_UI_ORIGIN = new Map([
  ['https://admin.kitsune.ai', 'https://api.kitsune.ai'],
  ['https://server-dev.kitsune-server-admin.pages.dev', 'https://kitsune-server-dev.up.railway.app'],
])
 ...
 *
 * Use when:
 * - The standalone admin UI serves more than one Kitsune environment from the same
 *   Pages deployment.
 *
 * Expects:
 * - The server-owned `/admin/*` redirect sets `api_server_url`.
 * - Only known Kitsune API origins and localhost development origins are accepted.
 *
 * Returns:
