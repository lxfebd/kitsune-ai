...
describe('ui-admin bootstrap context', () => {
  it('uses the trusted API server origin carried by standalone server redirects', () => {
    expect(resolveStandaloneServerAdminContext(
      'https://admin.kitsune.ai/users?api_server_url=https%3A%2F%2Fkitsune-server-dev.up.railway.app%2Fapi%2Fadmin',
    )).toEqual({
      apiServerUrl: 'https://kitsune-server-dev.up.railway.app',
      currentUrl: 'https://admin.kitsune.ai/users?api_server_url=https%3A%2F%2Fkitsune-server-dev.up.railway.app%2Fapi%2Fadmin',
    })
  })

  it('ignores untrusted API server origins from crafted standalone admin URLs', () => {
    expect(resolveStandaloneServerAdminContext(
      'https://admin.kitsune.ai/users?api_server_url=https%3A%2F%2Fevil.example',
    )).toBeNull()
  })
  ...
  it('defaults known standalone admin deployments to their API origins', () => {
    expect(defaultStandaloneApiServerUrl('https://admin.kitsune.ai')).toBe('https://api.kitsune.ai')
    expect(defaultStandaloneApiServerUrl('https://server-dev.kitsune-server-admin.pages.dev')).toBe('https://kitsune-server-dev.up.railway.app')
  })

  it('falls back to the standalone query context when the static placeholder script is still present', () => {
    document.body.innerHTML = '<script id="kitsune-server-admin-context" type="application/json">__KITSUNE_SERVER_ADMIN_CONTEXT__</script>'
    window.history.replaceState(
      null,
      '',
      '/admin/users?api_server_url=https%3A%2F%2Fkitsune-server-dev.up.railway.app',
    )

    expect(getServerAdminBootstrapContext()?.apiServerUrl).toBe('https://kitsune-server-dev.up.railway.app')
  })
})
