...
import type { Character, CreateCharacterPayload } from '@kitsune/stage-ui/types/character'
...
import { CreateCharacterSchema } from '@kitsune/stage-ui/types/character'
...
watch(() => props.character, (char) => {
  if (char) {
    ...
    const prompt = (type: 'system' | 'personality' | 'greetings') =>
      char.prompts?.find(p => p.language === 'en' && p.type === type)?.content || ''
    form.systemPrompt = prompt('system')
    form.personalityPrompt = prompt('personality')
    form.greetingsPrompt = prompt('greetings')
  }
  else {
    // Reset defaults
    form.characterId = ''
...
async function handleSubmit() {
  ...
  // Prompts are always sent as a full replacement set (server PATCH wipes & re-inserts).
  const prompts: { language: string, type: 'system' | 'personality' | 'greetings', content: string }[] = []
  if (form.systemPrompt)
    prompts.push({ language: 'en', type: 'system', content: form.systemPrompt })
  if (form.personalityPrompt)
    prompts.push({ language: 'en', type: 'personality', content: form.personalityPrompt })
  if (form.greetingsPrompt)
    prompts.push({ language: 'en', type: 'greetings', content: form.greetingsPrompt })
  ...
  try {
    if (props.character) {
      // PATCH updates base fields and replaces the prompt set (system/personality/greetings)
      // wholesale; capabilities/i18n edits are not part of this dialog yet.
...
const tabs = [
  { id: 'identity', label: '身份', icon: 'i-solar:user-id-bold-duotone' },
  { id: 'capabilities', label: '能力', icon: 'i-solar:cpu-bolt-bold-duotone' },
  { id: 'prompts', label: '提示词', icon: 'i-solar:chat-square-code-bold-duotone' },
            ...
            <div class="flex-1 overflow-y-auto p-6">
              ...
              <div v-show="activeTab === 'identity'" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                  <FieldInput v-model="form.characterId" label="角色ID" placeholder="例如 kitsune-core" required />
                  <FieldInput v-model="form.version" label="版本" placeholder="1.0.0" />
                </div>

                <FieldInput v-model="form.name" label="名称" placeholder="角色名称" required />
                <FieldInput v-model="form.description" label="描述" type="textarea" placeholder="简短描述..." />
                <FieldInput v-model="form.coverUrl" label="封面URL" placeholder="https://..." />
              </div>
              ...
              <div v-show="activeTab === 'capabilities'" class="space-y-6">
                <div class="space-y-4">
                  ...
                  <FieldInput v-model="form.llmModel" label="模型" placeholder="gpt-4o" />
                  <!-- Use number input for temperature properly -->
                  <div class="flex flex-col gap-1.5">
                    <label class="text-sm text-neutral-700 font-medium dark:text-neutral-300">温度</label>
                    <input
                ...
                <div class="space-y-4">
                  ...
                  <FieldInput v-model="form.ttsVoiceId" label="语音ID" placeholder="Voice ID" />
                  <div class="flex flex-col gap-1.5">
                    <label class="text-sm text-neutral-700 font-medium dark:text-neutral-300">语速</label>
                    <input
              ...
              <div v-show="activeTab === 'prompts'" class="space-y-6">
                <div class="space-y-4">
                  ...
                  <FieldInput v-model="form.systemPrompt" type="textarea" label="系统提示词" placeholder="定义角色的系统级行为与知识边界..." />
                </div>
                ...
                <div class="space-y-4">
                  ...
                  <FieldInput v-model="form.personalityPrompt" type="textarea" label="性格设定" placeholder="描述角色的性格、语气与说话风格..." />
                </div>
                ...
                <div class="space-y-4">
                  ...
                  <FieldInput v-model="form.greetingsPrompt" type="textarea" label="开场白" placeholder="角色初次见面时的问候语..." />
                </div>
              </div>
            </div>
          </div>
