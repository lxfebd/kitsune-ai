...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@kitsune/stage-layouts': resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-layouts', 'src')),
    },
  },
  server: {
    fs: {
  ...
  build: {
    ...
    // 浏览器端从不实例化该适配器（只走 engine-registry 的 listEngines），
    // 但 import 必须可解析。外部化后 import 留在 bundle 中但从不会执行。
  ...
  plugins: [
    ...
    SpaceCard({
      root: cwd(),
      title: 'Kitsune AI: Virtual Companion',
      emoji: '🧸',
      colorFrom: 'pink',
      colorTo: 'pink',
      sdk: 'static',
    ...
    ...((!env.S3_ENDPOINT || !env.S3_ACCESS_KEY_ID || !env.S3_SECRET_ACCESS_KEY)
      ...
      : [
          Basemove({
            prefix: env.STAGE_WEB_WARP_DRIVE_PREFIX || 'kitsune/stage-web/main/',
