...
async function prepareMcpConfig() {
  ...
  const config = {
    mcpServers: {
      computer_use: {
        command: 'pnpm',
        args: ['-F', '@kitsune/computer-use-mcp', 'start'],
        cwd: repoDir,
        enabled: true,
        env: mcpEnv,
      },
...
async function ensureSmokePrerequisites() {
  ...
  throw new Error([
    ...
    'Suggested command: pnpm -F \'@kitsune/stage-tamagotchi^...\' --if-present build',
  ].join(' '))
}
...
function startStage(debugPort: number, heartbeatLines: string[]): ChildProcessWithoutNullStreams {
  const stageProcess = spawn('pnpm', ['-F', '@kitsune/stage-tamagotchi', 'dev'], {
    cwd: repoDir,
    detached: true,
    env: {
      ...env,
