...
pnpm -F @kitsune/stage-tamagotchi update-test:generate \
  --root scripts/update-test/fixtures/server \
  --channel stable \
  --target aarch64-apple-darwin \
  --version 9.9.9-update-test.1
pnpm -F @kitsune/stage-tamagotchi update-test:server \
  --port 8787 \
  --root scripts/update-test/fixtures/server
```
...
```bash
pnpm -F @kitsune/stage-tamagotchi update-test:matrix
```
...
- When invoking the package scripts through `pnpm -F @kitsune/stage-tamagotchi`, treat `--root` as relative to `apps/stage-tamagotchi`, not the workspace root.
