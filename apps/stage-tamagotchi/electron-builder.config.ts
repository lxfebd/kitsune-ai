...
/** 检测本机构建 macOS 是否具备 Xcode 26+（决定 mac 图标用 .icon 还是 .icns 格式）。 */
...
// 可选资源：`godot-stage` 引擎二进制由上游 `engines/stage-tamagotchi-godot` 构建产出，
// `models/sherpa-onnx` 由 `scripts/download-asr-models.ps1` 下载生成。两者都是构建期可选
// 产物——仓库默认不包含它们，缺失时跳过打包，避免 electron-builder 因 from 路径不存在而直接报错。
// 对应代码在运行时按存在性回退（asr: userData 模型 → extraResources → 报错；godot-stage: 缺失即停用）。
const repoRoot = join(import.meta.dirname, '..', '..', '..')
const godotBuildDir = join(repoRoot, 'engines', 'stage-tamagotchi-godot', 'build')
const sherpaModelsDir = join(repoRoot, 'resources', 'models', 'sherpa-onnx')
const hasGodotBuild = existsSync(godotBuildDir)
const hasSherpaModels = existsSync(sherpaModelsDir)

if (!hasGodotBuild) {
  console.warn('[electron-builder/config] engines/stage-tamagotchi-godot/build not found — skipping godot-stage extraResources.')
}
if (!hasSherpaModels) {
  console.warn('[electron-builder/config] resources/models/sherpa-onnx not found — skipping ASR models extraResources.')
}

// mac 打包资源：`build/` 目录（上游模板遗留）不在仓库中。entitlements 与图标改为
// 存在性回退——缺失时 electron-builder 使用内建默认（并行签名/默认图标），而不是
// 引用必然不存在的路径导致打包失败。构建者在 macOS 上放置 build/ 资源后自动启用。
const entitlementsPath = join(import.meta.dirname, 'build', 'entitlements.mac.plist')
const macEntitlements = existsSync(entitlementsPath) ? entitlementsPath : undefined
const icnsPath = join(import.meta.dirname, useIconFormattedMacAppIcon ? 'icon.icon' : 'icon.icns')
const macIcon = existsSync(icnsPath) ? icnsPath : undefined
if (!macEntitlements) {
  console.warn('[electron-builder/config] build/entitlements.mac.plist not found — macOS build uses built-in entitlements.')
}
if (!macIcon) {
  console.warn('[electron-builder/config] mac icon (.icns/.icon) not found — macOS build falls back to the default Electron icon.')
}

const optionalGodotResources = hasGodotBuild
  ? [{
...
const optionalSherpaResources = hasSherpaModels
  ? [{
...
export default {
  ...
  extraResources: [
    ...optionalGodotResources,
    // NOTICE: 原 `resources/gpt-sovits` (GPT-SoVITS + Python runtime, ~6.6GB) 已从安装包移除，
    // 改为运行时插件按需下载（userData/runtime-plugins/tts-gptsovits），见 services/kitsune/runtime-plugins。
    ...optionalSherpaResources,
  ...
  mac: {
    // NOTICE: `build/` 目录（上游模板遗留）不在仓库中，entitlements 缺失时回退内建值。
