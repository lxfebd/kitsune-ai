...
import type { DisplayModel } from '@kitsune/stage-ui/stores/display-models'
...
import { ModelSettingsPanel } from '@kitsune/stage-ui/components/scenarios/settings/model-settings'
...
function inferModelFileName(model: DisplayModel) {
  ...
  try {
    ...
    // 无效 URL 时使用默认文件名 fallback（URL 解析失败是预期行为）
  }
...
async function readSceneInputData(model: DisplayModel) {
  ...
  if (!response.ok)
    throw new Error(`获取模型资源失败 (${response.status} ${response.statusText})`)
...
async function refreshGodotStageStatus() {
  try {
    ...
    godotStageError.value = errorMessageFrom(error) ?? '查询 Godot 舞台状态失败。'
  }
}
...
async function refreshGodotViewSnapshot(epoch = godotSessionEpoch) {
  try {
    ...
    setGodotViewError(error, '查询 Godot 舞台视图状态失败。')
  }
}
...
async function applyGodotViewPatchNow(patch: StageViewPatch) {
  ...
  try {
    ...
    setGodotViewError(error, '应用 Godot 舞台视图补丁失败。')
  }
}
...
async function handleGodotViewSnapshotRequest(epoch = godotSessionEpoch) {
  try {
    ...
    setGodotViewError(error, '请求 Godot 舞台视图快照失败。')
  }
}
...
async function syncGodotSceneInput(model: DisplayModel) {
  ...
  try {
    ...
    godotStageError.value = errorMessageFrom(error) ?? '将模型输入应用到 Godot 舞台失败。'
  }
}
...
async function handleGodotStageToggle() {
  ...
  try {
    ...
    godotStageError.value = errorMessageFrom(error) ?? '切换 Godot 舞台模式失败。'
