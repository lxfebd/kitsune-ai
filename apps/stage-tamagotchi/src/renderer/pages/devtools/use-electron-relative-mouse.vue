...
import { useElectronMouse, useElectronRelativeMouse, useElectronWindowBounds } from '@kitsune/electron-vueuse'
    ...
    <div class="relative border-2 border-neutral-300 rounded bg-white p-8 dark:border-neutral-700 dark:bg-neutral-950" style="height: 400px;">
      ...
      <div class="absolute inset-8 border-2 border-primary-500 rounded bg-primary-50/20 dark:bg-primary-950/20">
        <div class="absolute left-0 text-xs text-primary-600 font-semibold -top-6">
          {{ t('tamagotchi.settings.devtools.pages.relative-mouse.window') }} ({{ windowBounds.width.value }}×{{ windowBounds.height.value }})
        </div>
        ...
        <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          ...
          <div class="absolute left-1/2 top-6 whitespace-nowrap text-[10px] text-neutral-500 -translate-x-1/2">
            {{ t('tamagotchi.settings.devtools.pages.relative-mouse.center') }} ({{ Math.round(windowBounds.width.value / 2) }}, {{ Math.round(windowBounds.height.value / 2) }})
          </div>
        </div>
      ...
      <div class="absolute left-0 text-xs text-neutral-600 -bottom-6 dark:text-neutral-400">
        {{ t('tamagotchi.settings.devtools.pages.relative-mouse.cursor-hint') }}
      </div>
    </div>
  </div>
</template>
...
meta:
  layout: settings
  titleKey: tamagotchi.settings.devtools.pages.relative-mouse.title
