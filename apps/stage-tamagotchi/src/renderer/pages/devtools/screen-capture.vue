...
import type { SerializableDesktopCapturerSource } from '@kitsune/electron-screen-capture'
import type { SourcesOptions } from 'electron'

import { useElectronScreenCapture } from '@kitsune/electron-screen-capture/vue'
import { Button, SelectTab } from '@kitsune/ui'
...
const categoryOptions = [
  { label: 'Applications', value: 'applications', icon: 'i-solar:window-frame-line-duotone', labelKey: 'tamagotchi.settings.devtools.pages.screen-capture.applications' },
  { label: 'Displays', value: 'displays', icon: 'i-solar:screencast-2-line-duotone', labelKey: 'tamagotchi.settings.devtools.pages.screen-capture.displays' },
  { label: 'Devices', value: 'devices', icon: 'i-solar:smartphone-2-line-duotone', labelKey: 'tamagotchi.settings.devtools.pages.screen-capture.devices' },
]
...
const refetchLabel = computed(() => {
  if (isInitialLoading.value)
    return t('tamagotchi.settings.devtools.pages.screen-capture.loading')
  return isRefetching.value ? t('tamagotchi.settings.devtools.pages.screen-capture.refetching') : t('tamagotchi.settings.devtools.pages.screen-capture.refetch')
})
...
function getShareLabel(source: ScreenCaptureSource) {
  ...
  if (isDeviceSource(source))
    return t('tamagotchi.settings.devtools.pages.screen-capture.share-device')
      ...
      >
        ...
        >
          <div :class="['flex', 'items-center', 'gap-2']">
            ...
            <div>{{ t('tamagotchi.settings.devtools.pages.screen-capture.capturing') }}</div>
          </div>
          <div :class="['flex', 'w-full', 'items-center', 'gap-3', 'overflow-x-auto']">
            ...
            >
              ...
              >
                ...
                <div class="text-sm">
                  {{ t('tamagotchi.settings.devtools.pages.screen-capture.stop') }}
                </div>
              </div>
              <video
                autoplay
        ...
        >
          <div :class="['flex', 'w-full', 'items-center', 'gap-3']">
            <SelectTab
              ...
              :options="categoryOptions.map(option => ({
                ...option,
                label: `${t(option.labelKey)} (${sourceCounts[option.value as SourceCategory]})`,
              }))"
              :class="['flex-1']"
            />
            <Button
          ...
          >
            <div :class="['flex', 'items-center', 'gap-2']">
              ...
              <span>{{ t('tamagotchi.settings.devtools.pages.screen-capture.loading-sources') }}</span>
            </div>
          </div>
          ...
          >
            ...
            <div>{{ t('tamagotchi.settings.devtools.pages.screen-capture.no-sources') }}</div>
            <div class="text-xs text-neutral-400">
              {{ t('tamagotchi.settings.devtools.pages.screen-capture.try-switching') }}
            </div>
          </div>
        </div>
      </div>
