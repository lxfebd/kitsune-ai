...
import type { BugReportDialogSubmitPayload } from '@kitsune/stage-ui/components'

import type { ElectronUpdaterChannel } from '../../shared/eventa'

import semver from 'semver'

import { useElectronAutoUpdater, useElectronEventaInvoke } from '@kitsune/electron-vueuse'
import { AboutContent, BugReportDialog, createBugReportPageContext, MarkdownRenderer } from '@kitsune/stage-ui/components'
import { useBreakpoints } from '@kitsune/stage-ui/composables'
import { useSharedAnalyticsStore } from '@kitsune/stage-ui/stores/analytics'
import { Button, ContainerError, DoubleCheckButton, FieldSelect, Progress } from '@kitsune/ui'
  ...
  >
    <div :class="['mx-auto max-w-[min(960px,calc(100%-2rem))]', 'p-6']">
      <AboutContent
        ...
        :highlight="t('tamagotchi.stage.about.highlight')"
        ...
        <template #before-build-info>
          ...
          >
            ...
            <div :class="['flex flex-col gap-4']">
              ...
              >
                {{ t('tamagotchi.stage.about.update.windows-admin-warning') }}
              ...
              <div v-if="updateState.status === 'available'" :class="['flex flex-col gap-4']">
                ...
                <div
                  ...
                  :class="['text-sm rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-amber-700 dark:text-amber-200']"
                >
                  {{ t('tamagotchi.stage.about.update.downgrade-warning') }}
                </div>
                <div>
                  <Button
                    variant="primary"
              ...
              <div v-else-if="updateState.status === 'downloaded'" :class="['flex flex-col gap-4']">
                ...
                <div
                  ...
                  :class="['text-sm rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-amber-700 dark:text-amber-200']"
                >
                  {{ t('tamagotchi.stage.about.update.downgrade-downloaded-warning') }}
                </div>
                <div>
                  <DoubleCheckButton
                    variant="primary"
    ...
    <DialogRoot v-if="isDesktop" v-model:open="showChangelog">
      <DialogPortal>
        ...
        <DialogContent class="fixed left-1/2 top-1/2 z-[9999] max-h-[85vh] max-w-2xl w-[90vw] flex flex-col rounded-2xl bg-white p-6 shadow-xl outline-none backdrop-blur-md -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:bg-neutral-900">
          ...
          <div class="min-h-0 flex-1 overflow-y-auto border border-neutral-200 rounded-lg bg-neutral-50 p-4 dark:border-neutral-800 bg-white/50 dark:bg-white/[0.03] backdrop-blur-2xl border border-black/[0.06] dark:border-white/[0.06] rounded-2xl transition-all duration-300 hover:shadow-sm hover:shadow-black/[0.02] dark:hover:shadow-black/[0.08]">
