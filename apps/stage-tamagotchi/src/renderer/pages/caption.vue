...
import { useElectronEventaContext, useElectronMouseAroundWindowBorder, useElectronMouseInWindow } from '@kitsune/electron-vueuse'
import { createFadeAnimator, PoppinText } from '@kitsune/stage-ui/components'
...
const { data } = useBroadcastChannel<CaptionChannelEvent, CaptionChannelEvent>({ name: 'kitsune-caption-overlay' })
...
onMounted(async () => {
  try {
    ...
    console.error('[caption] 获取挂载状态失败:', e)
  }
  ...
  try {
    ...
    console.error('[caption] 注册跟随窗口事件失败:', e)
  }
  ...
  try {
    ...
    console.error('[caption] 监听广播频道失败:', e)
  }
})
  ...
  <div class="pointer-events-none relative h-full w-full flex items-end justify-center">
    ...
    >
      <div
        ...
        :title="t('tamagotchi.stage.caption.drag-to-move')"
      >
