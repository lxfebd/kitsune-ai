...
vi.mock('@kitsune/stage-ui/stores/chat/session-store', () => ({
  useChatSessionStore: () => ({
...
vi.mock('@kitsune/stage-ui/stores/chat/stream-store', () => ({
  useChatStreamStore: () => ({
    streamingMessage: ref({ role: 'assistant', content: '', slices: [], tool_results: [] }),
  }),
}))

vi.mock('@kitsune/stage-ui/stores/chat', () => ({
  useChatOrchestratorStore: () => ({
    sending: ref(false),
    ingest: mockState.ingest,
    // NOTICE: persona 缓存方法由 chat.ts 暴露，chat-sync.ts executeIngest 会调用
    // isPersonaCacheValid 做缓存命中检查。测试 mock 必须补全这两个方法，否则
    // chat-sync.ts 调用时抛 TypeError: isPersonaCacheValid is not a function。
    isPersonaCacheValid: vi.fn(() => false),
    setCachedPersonaPrompt: vi.fn(),
    getCachedPersonaPrompt: vi.fn(() => null),
  }),
}))

vi.mock('@kitsune/stage-ui/stores/chat/maintenance', () => ({
  useChatMaintenanceStore: () => ({
    cleanupMessages: vi.fn(),
  }),
}))

vi.mock('@kitsune/stage-ui/stores/providers', () => ({
  useProvidersStore: () => ({
    getProviderInstance: vi.fn(async () => ({ id: 'provider' })),
  }),
}))

vi.mock('@kitsune/stage-ui/stores/modules/active-model', () => ({
  useActiveModelStore: () => ({
...
describe('useChatSyncStore', async () => {
  ...
  it('stores command ingest errors in authority session history', async () => {
    ...
    const peer = new MockBroadcastChannel('kitsune:stage-tamagotchi:chat-sync')
    peer.postMessage({
      type: 'command',
      requestId: 'req-1',
      senderId: 'peer',
  ...
  it('keeps the follower chat window on its local session while applying remote snapshots', async () => {
    ...
    const authority = new MockBroadcastChannel('kitsune:stage-tamagotchi:chat-sync')
