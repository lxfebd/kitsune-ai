...
import { webTools } from './tools/builtin/web-tools'
...
export const useChatSyncStore = defineStore('stage-tamagotchi:chat-sync', () => {
  ...
  function resolveTools(toolset?: ToolsetId) {
    const toolsetRegistry: Record<string, () => Promise<Awaited<ReturnType<typeof widgetsTools>>>> = {
      widgets: async () => {
        const [w, we] = await Promise.all([widgetsTools(), weatherTools()])
        return [...w, ...we]
      },
      artistry: async () => {
        const [ai, wi, we, wb] = await Promise.all([
          imageJournalTools(),
          widgetsTools(),
          weatherTools(),
          webTools(),
      ...
      automation: async () => {
        const [d, w, we, wb] = await Promise.all([
