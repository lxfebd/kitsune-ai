    ...
    const candidateId = selectDesktopOverlaySmokeCandidateId({
      lastGroundingSnapshot: {
        targetCandidates: [
          ...
          { id: 'smoke', label: 'Kitsune Desktop Overlay Smoke Button', role: 'button', source: 'chrome_dom' },
        ],
      },
    })
  ...
  it('requires the smoke button to come from chrome_dom', () => {
    expect(() => {
      selectDesktopOverlaySmokeCandidateId({
        lastGroundingSnapshot: {
          targetCandidates: [
            { id: 'ax-smoke', label: 'Kitsune Desktop Overlay Smoke Button', role: 'button', source: 'ax' },
          ],
        },
      })
    }).toThrow('desktop_observe did not return the Kitsune Desktop Overlay Smoke Button chrome_dom candidate')
  })
