...
export function selectDesktopOverlaySmokeCandidateId(runState: Record<string, unknown>): string {
  ...
  const selected = candidates.find((candidate) => {
    return candidate.source === 'chrome_dom'
      && String(candidate.label || '').includes('Kitsune Desktop Overlay Smoke Button')
  })

  const id = typeof selected?.id === 'string' ? selected.id : ''
  if (!id)
    throw new Error('desktop_observe did not return the Kitsune Desktop Overlay Smoke Button chrome_dom candidate')
