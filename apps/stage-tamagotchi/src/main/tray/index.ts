...
import { isRendererUnavailable } from '@kitsune/electron-vueuse/main'
  ...
  once(() => {
    ...
    const rebuildContextMenu = debounce((): void => {
      ...
      const contextMenu = Menu.buildFromTemplate([
        ...
        { type: 'separator' },
        ...(env.ELECTRON_RENDERER_URL || env.MAIN_APP_DEBUG || env.APP_DEBUG)
          ? [
              { type: 'header', label: params.i18n.t('tamagotchi.electron.tray.menu.labels.label.devtools') },
              { label: params.i18n.t('tamagotchi.electron.tray.menu.labels.label.troubleshoot_beatsync'), click: () => params.beatSyncBgWindow.ensureWindow().then(window => window.webContents.openDevTools({ mode: 'detach' })) },
              { type: 'separator' },
            ] as const
          : [],
    ...
    appTray.setToolTip('Project Kitsune')
