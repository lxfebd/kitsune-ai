 ...
 * - Stale flags
 *
 * Gated by KITSUNE_DESKTOP_OVERLAY=1 environment variable.
...
import { isDesktopOverlayEnabled } from './is-desktop-overlay-enabled'
...
export function isDesktopOverlayPollHeartbeatEnabled(): boolean {
  return env.KITSUNE_DESKTOP_OVERLAY_POLL_HEARTBEAT === '1'
}
 ...
 *
 * Returns null if KITSUNE_DESKTOP_OVERLAY is not set.
 */
...
/**
 * Set overlay interactive mode (toggle click-through).
 * - interactive=true: allow user click on overlay elements
 * - interactive=false: click-through (render ghost pointer only)
 */
