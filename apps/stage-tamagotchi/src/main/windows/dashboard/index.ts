...
export async function setupDashboardWindow(params: {
  ...
  const windowConfig = getConfig().windows?.find(w => w.title === 'Kitsune AI Dashboard' && w.tag === 'dashboard')

  const window = new BrowserWindow({
    title: 'Kitsune AI Dashboard',
  ...
  if (env.ELECTRON_RENDERER_URL || env.MAIN_APP_DEBUG || env.APP_DEBUG) {
    try {
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => w.title === 'Kitsune AI Dashboard' && w.tag === 'dashboard')
    ...
    if (existingConfigIndex === -1) {
      ...
      const windowConfig = defu(config.windows[existingConfigIndex], { title: 'Kitsune AI Dashboard', tag: 'dashboard' })
