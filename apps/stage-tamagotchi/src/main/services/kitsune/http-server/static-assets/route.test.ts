...
describe('createStaticAssetRoute', () => {
  ...
  it('returns 401 when cookie is missing', async () => {
    const app = new H3()
    app.get('/_kitsune/extensions/**', createStaticAssetRoute({
      authorize: async () => ({
        ok: false,
        error: new HttpError({
          status: 401,
    ...
    const response = await fetch(`http://127.0.0.1:${port}/_kitsune/extensions/a/sessions/s1/ui/index.html`)
  ...
  it('returns 405 with security headers when method is not allowed', async () => {
    const app = new H3()
    app.use('/_kitsune/extensions/**', createStaticAssetRoute({
      authorize: async () => ({
        ok: false,
        error: new HttpError({
          status: 401,
  ...
  it('uses custom getType resolver and sets nosniff', async () => {
      ...
      authorize: async ({ cookieValue }) => {
        ...
        return {
          ...
          session: {
            ...
            cookiePath: '/_kitsune/extensions/a/sessions/s1/ui',
    ...
    const response = await fetch(`http://127.0.0.1:${port}/_kitsune/extensions/a/sessions/s1/ui/module.wasm`, {
