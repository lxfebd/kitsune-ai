...
export function createStaticAssetSessionCookieName(assetSessionId: string) {
  return `kitsune_extension_asset_session_${assetSessionId}`
}

function createCookiePath(extensionId: string, assetSessionId: string) {
  return `/_kitsune/extensions/${encodeURIComponent(extensionId)}/sessions/${encodeURIComponent(assetSessionId)}/ui`
}
