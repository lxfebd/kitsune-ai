...
describe('createStaticAssetService', () => {
  ...
  it('accepts pathPrefix relative to /ui route segment', async () => {
    ...
    const extensionId = 'kitsune-plugin-game-chess'
    const version = '1.0.0'
    ...
    const response = await fetch(`${baseUrl}/_kitsune/extensions/${extensionId}/sessions/${session.assetSessionId}/ui/assets/app.js`, {
      headers: {
  ...
  it('serves HEAD requests with valid cookie auth refresh and empty body', async () => {
    ...
    const response = await fetch(`${server.getBaseUrl()}/_kitsune/extensions/${extensionId}/sessions/${session.assetSessionId}/ui/assets/app.js`, {
      headers: {
        cookie: `${session.cookieName}=${session.cookieValue}`,
      },
      method: 'HEAD',
  ...
  it('returns 405 with security headers for POST requests', async () => {
    ...
    const response = await fetch(`${server.getBaseUrl()}/_kitsune/extensions/${extensionId}/sessions/session-1/ui/assets/app.js`, {
      method: 'POST',
    })
  ...
  it('rejects a previously valid URL and cookie after revocation', async () => {
    ...
    const url = `${server.getBaseUrl()}/_kitsune/extensions/${extensionId}/sessions/${session.assetSessionId}/ui/assets/app.js`
    const headers = {
      cookie: `${session.cookieName}=${session.cookieValue}`,
    }
  ...
  it('returns 404 for missing in-root assets with a valid session', async () => {
    ...
    const response = await fetch(`${server.getBaseUrl()}/_kitsune/extensions/${extensionId}/sessions/${session.assetSessionId}/ui/assets/missing.js`, {
      headers: {
        cookie: `${session.cookieName}=${session.cookieValue}`,
      },
    })
