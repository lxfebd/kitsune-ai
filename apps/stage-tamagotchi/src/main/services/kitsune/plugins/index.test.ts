...
const extensionManifestFileName = 'extension.kitsune.json'
...
function createDynamicModuleManifest(entrypoint: string, id = 'test-dynamic-module'): ExtensionManifestV1 {
  const providersCapability = 'kitsune:plugin-sdk:apis:protocol:resources:providers:list-providers'
  const permissions: ModulePermissionDeclaration = {
    apis: [
      { key: 'kitsune:plugin-sdk:apis:protocol:capabilities:wait', actions: ['invoke'] },
      { key: providersCapability, actions: ['invoke'] },
      { key: 'kitsune:plugin-sdk:apis:client:kits:list', actions: ['invoke'] },
      { key: 'kitsune:plugin-sdk:apis:client:kits:get-capabilities', actions: ['invoke'] },
      { key: 'kitsune:plugin-sdk:apis:client:bindings:list', actions: ['invoke'] },
      { key: 'kitsune:plugin-sdk:apis:client:bindings:announce', actions: ['invoke'] },
    ],
    resources: [
      { key: providersCapability, actions: ['read'] },
      { key: 'kitsune:plugin-sdk:resources:kits', actions: ['read'] },
      { key: 'kitsune:plugin-sdk:resources:bindings', actions: ['read'] },
      { key: 'kitsune:plugin-sdk:resources:kits:kit.widget:bindings', actions: ['read', 'write'] },
    ],
    capabilities: [
...
function createExtensionGameletKitManifest(entrypoint: string, id = 'test-extension-gamelet-kit'): ExtensionManifestV1 {
  return {
    ...
    permissions: {
      ...
      resources: [
        { key: 'kitsune:plugin-sdk:resources:kits:kit.gamelet:bindings', actions: ['write'] },
      ],
    },
    entrypoints: {
      electron: entrypoint,
    ...
    if (respondToRequests && request && typeof request === 'object' && !Array.isArray(request) && typeof (request as Record<string, unknown>).requestId === 'string') {
      ...
      queueMicrotask(() => {
        publishWidgetEvent(payload.id, {
          route: {
            namespace: 'kitsune.plugin.gamelet',
            name: 'response',
          },
          payload: {
            requestId,
...
describe('setupExtensionHost', () => {
  ...
  it('loads enabled plugins with absolute manifest entrypoints outside the plugin directory', async () => {
    const externalDir = await mkdtemp(join(tmpdir(), 'kitsune-plugin-external-'))
  ...
  it('loads the chess-like demo plugin and exposes a gamelet module snapshot', async () => {
    const pluginDir = join(pluginsDir, 'kitsune-plugin-game-chess')
    await mkdir(pluginDir, { recursive: true })
    await writeFile(
      join(pluginDir, extensionManifestFileName),
      JSON.stringify({
        ...
        entrypoints: {
          electron: './kitsune-plugin-game-chess.mjs',
        },
      }, null, 2),
    )
    await writeFile(join(pluginDir, 'kitsune-plugin-game-chess.mjs'), [
      ...
      '        resources: [{ key: "kitsune:plugin-sdk:resources:kits:kit.gamelet:bindings", actions: ["write"] }],',
      '      },',
      '    })',
    ...
    await invokeSetEnabled({ extensionId: 'kitsune-plugin-game-chess', enabled: true })

    const registry = await invokeLoadEnabled()
    const plugin = registry.plugins.find(item => item.extensionId === 'kitsune-plugin-game-chess')
    expect(plugin).toEqual(expect.objectContaining({ enabled: true, loaded: true }))
    ...
    expect(snapshot.modules).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        ownerExtensionId: 'kitsune-plugin-game-chess',
        kitId: 'kit.gamelet',
          ...
          widget: expect.objectContaining({
            ...
            iframe: expect.objectContaining({
              ...
              src: expect.stringMatching(
                /^http:\/\/127\.0\.0\.1:\d+\/_kitsune\/extensions\/kitsune-plugin-game-chess\/sessions\/[\w-]{10,}\/ui\/index\.html$/,
              ),
  ...
  it('rewrites plugin widget iframe asset URLs in inspect snapshots', async () => {
        ...
        config: expect.objectContaining({
          ...
          widget: expect.objectContaining({
            iframe: expect.objectContaining({
              assetPath: './ui/index.html',
              src: expect.stringMatching(
                /^http:\/\/127\.0\.0\.1:\d+\/_kitsune\/extensions\/test-plugin-widget-asset-url\/sessions\/[\w-]{10,}\/ui\/index\.html$/,
              ),
              sandbox: 'allow-scripts allow-same-origin allow-forms allow-popups',
            }),
          }),
    ...
    const outsideSessionUrl = `${iframeUrl.origin}/_kitsune/extensions/test-plugin-widget-asset-url/ui/private/secret.txt`
  ...
  it('injects gamelet orchestration methods backed by the widget manager', async () => {
    ...
    await mkdir(pluginDir, { recursive: true })
    await linkWorkspacePackageForPlugin(pluginDir, '@kitsune/plugin-sdk')
    await linkWorkspacePackageForPlugin(pluginDir, '@kitsune/plugin-sdk-tamagotchi')
    const entrypointPath = await writeEntrypoint({
      dir: pluginDir,
      name: 'test-extension-gamelet-orchestration.ts',
      contents: [
        'import { defineExtension } from \'@kitsune/plugin-sdk\'',
        'import { gameletKit } from \'@kitsune/plugin-sdk-tamagotchi\'',
        '',
        'export default defineExtension({',
  ...
  it('closes mounted gamelets when the owning extension session stops', async () => {
    ...
    const entrypointPath = await writeEntrypoint({
      ...
      contents: [
        'import { createModule, defineExtension } from \'@kitsune/plugin-sdk\'',
        'import { createGamelet } from \'@kitsune/plugin-sdk-tamagotchi/kits/gamelet\'',
        '',
        'export default defineExtension({',
