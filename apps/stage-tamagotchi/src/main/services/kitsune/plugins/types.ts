...
export interface ExtensionHostService {
  ...
  /**
   * Performs deferred initialization: starts the extension asset HTTP server,
   * refreshes manifests, and loads enabled extensions.
   *
   * Called after the main window is created so the injeca dependency chain
   * is not blocked by plugin loading during startup.
   */
  init: () => Promise<void>
  /**
   * Returns the latest extension registry snapshot, refreshing manifests first.
 ...
 * - `path` points to `extension.kitsune.json`
