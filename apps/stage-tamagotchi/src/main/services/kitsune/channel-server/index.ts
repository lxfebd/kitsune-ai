import type { Server, ServerOptions } from '@kitsune/server-runtime/server'
...
function isTrustedServerChannelCertificate(request: ServerChannelCertificateVerifyRequest): boolean {
  ...
  return request.certificate.subject.commonName === 'localhost'
    && issuer.commonName === 'Kitsune'
    && issuer.country === 'US'
    && issuer.locality === 'Local'
    && issuer.organizations.includes('Kitsune')
}
...
async function installCACertificate(caCert: string) {
  ...
  try {
    if (platform === 'darwin') {
      ...
      const caFileName = 'kitsune-websocket-ca.crt'
      try {
...
export async function setupServerChannel(params: { lifecycle: Lifecycle }): Promise<Server> {
  ...
  return {
    ...
    onPeerOpen: handler => serverChannel.onPeerOpen(handler),
    onPeerClose: handler => serverChannel.onPeerClose(handler),
