...
async function resolveGodotProjectPath() {
  ...
  while (true) {
    ...
    try {
      ...
      // 未找到则继续向上搜索（project.godot 不存在于当前目录）
    }
...
async function resolveExportedGodotBinary(): Promise<string | undefined> {
  ...
  try {
    ...
    // 导出的 Godot 二进制文件不存在，返回 undefined 让调用方尝试其他路径
    return undefined
  }
}
...
export function createGodotStageManager(): GodotStageManager {
  ...
  async function stopSocketRuntime() {
    ...
    await runtime.server.close(true).catch((error) => {
      log.withError(error).warn('Failed to close Godot stage WebSocket server')
    })
  }
      ...
      return await lifecycleMutex.runExclusive(async () => {
        ...
        try {
          ...
          const sidecarArgs = [
            ...
            '--',
            `--kitsune-ws-url=${websocketUrl}`,
            `--kitsune-storage-root=${resolveGodotStageStorageRoot()}`,
          ]
        ...
        try {
          ...
          if (!exited) {
            ...
            await exitPromise.catch((error) => {
              log.withError(error).warn('Godot stage process exit wait failed')
            })
          }
        }
        catch (error) {
