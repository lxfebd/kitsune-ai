...
/**
 * Replicate run() 可返回的输出形态：
 * - string：直接是图片 URL
 * - 带有 url 方法或 url 字符串的对象（FileOutput 或简化版本）
 */
type ReplicateOutput = string | { url: (() => { href: string }) | string }
...
export class ReplicateProvider implements ArtistryProvider {
  ...
  async generate(request: ArtistryRequest): Promise<ArtistryJob> {
    ...
    if (request.extra) {
      ...
      const { prompt: _overriddenPrompt, ...safeRest } = rest
    ...
    const replacePlaceholders = (obj: unknown): unknown => {
      ...
      if (typeof obj === 'object' && obj !== null) {
        ...
        for (const key in record)
          newObj[key] = replacePlaceholders(record[key])
        return newObj
      }
      return obj
    }
  ...
  private async runGeneration(jobId: string, model: `${string}/${string}`, input: object) {
    ...
    try {
      ...
      if (items.length > 0) {
        const first: ReplicateOutput = items[0]
        let imageUrl: string | undefined

        // Case 1 & 2: Object with url property (method returning URL object, or string)
        if (typeof first === 'object' && first !== null && 'url' in first) {
          const urlVal = first.url
          if (typeof urlVal === 'function') {
            imageUrl = urlVal().href
          }
