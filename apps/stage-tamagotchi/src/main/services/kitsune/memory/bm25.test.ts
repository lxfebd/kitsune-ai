...
import { createBM25Index as createNativeIndex } from '@kitsune/bm25-native'
...
describe('createBM25Index', () => {
  ...
  it('native implementation should produce identical scores to the TS reference', () => {
    const docs = [
      ['a', '我喜欢 咖啡 编程 周末 爬山 猫 音乐 电影 旅游 美食'],
      ['b', '用户今天 去了 医院 检查 身体 健康 锻炼 跑步'],
      ['c', '量子计算 未来 技术 趋势 人工智能 大模型 语言'],
      ['d', '周末 咖啡 店 看书 学习 效率 提高 专注 时间管理'],
      ['e', '宠物 猫 狗 喂养 照顾 疫苗 健康 检查 兽医'],
    ]
    const queries = ['咖啡 周末', '健康 检查', '大模型', '宠物 猫', '学习 效率', '不存在的词']

    const native = createNativeIndex()
    const ts = createBM25Index()
    ...
    for (const q of queries) {
      const n = native.search(q, 3)
      const t = ts.search(q, 3)
      expect(n.map(r => `${r.id}:${r.score.toFixed(6)}`)).toEqual(t.map(r => `${r.id}:${r.score.toFixed(6)}`))
    }

    // remove 后依旧一致
    native.remove('e')
    ts.remove('e')
    expect(native.search('宠物', 2).map(r => r.id)).toEqual(ts.search('宠物', 2).map(r => r.id))
  })
})