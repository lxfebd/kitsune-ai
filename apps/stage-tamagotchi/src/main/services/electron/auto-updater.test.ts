...
describe('setupAutoUpdater', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.KITSUNE_UPDATE_CHANNEL
  ...
  it('supports explicit stable lane selection for future dynamic channel switching', async () => {
    process.env.KITSUNE_UPDATE_CHANNEL = 'stable'
    mockGitHubReleasesFetch([
      { tag_name: 'v0.9.0-beta.6', draft: false, prerelease: true },
  ...
  it.each(laneMatrix)('supports KITSUNE_UPDATE_CHANNEL override for lane=%s', async (lane) => {
    appMock.getVersion.mockReturnValue('0.9.0-alpha.2')
    process.env.KITSUNE_UPDATE_CHANNEL = lane
  ...
  it.each(bundleVersions)('uses bundled version lane when no KITSUNE_UPDATE_CHANNEL (bundle=%s)', async (bundleVersion) => {
  ...
  it('reports only authoritative diagnostics fields', async () => {
    ...
    expect(service.state.diagnostics).toEqual(expect.objectContaining({
      ...
      logFilePath: expect.stringMatching(/kitsune-updater[\\/]updater-log\.txt$/),
