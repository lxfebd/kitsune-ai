...
const UPDATER_DEBUG_CACHE_DIR = join(getCacheRoot(), 'kitsune-updater')
const UPDATER_LOG_FILE = join(UPDATER_DEBUG_CACHE_DIR, 'updater-log.txt')
const OFFICIAL_UPDATER_CACHE_DIR = join(getCacheRoot(), 'ai.kitsune.updater')
const LEGACY_OFFICIAL_UPDATER_CACHE_DIR = join(getLegacyCacheRoot(), 'ai.kitsune.updater')
...
async function logToFile(level: 'INFO' | 'WARN' | 'ERROR' | 'DEBUG', message: string) {
  await mkdir(UPDATER_DEBUG_CACHE_DIR, { recursive: true }).catch((error) => {
    console.error(`[auto-updater] Failed to create updater cache directory:`, error)
  })
  await appendFile(UPDATER_LOG_FILE, `${new Date().toISOString()} [${level}] ${message}\n`).catch((error) => {
    console.error(`[auto-updater] Failed to write updater log file:`, error)
  })
}
...
export function setupAutoUpdater(options: AutoUpdaterOptions = {}): AutoUpdater {
  ...
  async function prepareGitHubGenericFeed() {
    if (autoUpdater instanceof MockAutoUpdater)
      return
    if (activeFeedUrlOverride)
      return
  ...
  return {
    ...
    subscribe(callback) {
      ...
      try {
        ...
        // 单个 hook 失败不应影响其他 hook
        console.warn('[auto-updater] Subscriber callback failed:', error)
      }
