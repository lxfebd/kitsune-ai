...
import { ShortcutFailureReasons } from '@kitsune/stage-shared/global-shortcut'
...
/**
 * Safely access UiohookKey properties by name.
 * UiohookKey is a const enum from uiohook-napi; its runtime shape is
 * Record<string, number> but TypeScript type definitions may not expose
 * all property names. This helper narrows the cast to a single location.
 */
function getUiohookKeyCode(keyName: string): number | undefined {
  const keyMap = UiohookKey as unknown as Record<string, number>
...
function buildKeycodeMap(): Record<ShortcutKey, number> {
  ...
  for (let i = 0; i < 26; i++) {
    ...
    const code = getUiohookKeyCode(letter)
    if (code !== undefined)
      map[`Key${letter}`] = code
  }

  for (let i = 0; i <= 9; i++) {
    const code = getUiohookKeyCode(String(i))
    if (code !== undefined)
      map[`Digit${i}`] = code
  }

  for (let i = 1; i <= 24; i++) {
    const code = getUiohookKeyCode(`F${i}`)
    if (code !== undefined)
      map[`F${i}`] = code
  }
  ...
  for (const [w3c, uioName] of Object.entries(named)) {
    const code = getUiohookKeyCode(uioName as string)
    if (code !== undefined)
      map[w3c] = code
  }
