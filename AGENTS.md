...
- **UI/Shared Packages**:
  ...
  - `packages/stage-ui-pixi`: Pixi.js rendering components & composables.
  ...
  - Server channel: `packages/server-runtime`, `packages/server-sdk`, `packages/server-shared` (power `services/` and `integrations/`).
  - Legacy: 早期曾用 Tauri（`crates/`）做桌面端；当前桌面端统一为 Electron（`apps/stage-tamagotchi`）。
...
- **Stage UI internals** (`packages/stage-ui/src`)
  ...
  - Modules: `stores/modules/` (orchestration building blocks).
...
- `packages/stage-ui`: Core stage business components/composables/stores.
  ...
  - `src/stores/modules/`: orchestration modules.
...
- Legacy desktop: 早期 Tauri 桌面已废弃；桌面端统一为 Electron（`apps/stage-tamagotchi`）。
...
> Use pnpm workspace filters to scope tasks. Examples below are generic; replace the filter with the target workspace name (e.g. `@kitsune/stage-tamagotchi`, `@kitsune/stage-web`, `@kitsune/stage-ui`, etc.).

- **Typecheck**
  - `pnpm -F <package.json name> typecheck`
  - Example: `pnpm -F @kitsune/stage-tamagotchi typecheck` (runs `tsc` + `vue-tsc`).
- **Unit tests (Vitest)**
  ...
  - Workspace: `pnpm -F <package.json name> exec vitest run`
    e.g. `pnpm -F @kitsune/stage-tamagotchi exec vitest run`
...
- **Build**
  ...
  - Example: `pnpm -F @kitsune/stage-tamagotchi build` (typecheck + electron-vite build).
...
- Build primitives on `@kitsune/ui` (reka-ui) instead of raw DOM; see `packages/ui/src/components/Form` for implementation patterns.
- **When adding or updating components in `packages/ui`**, keep the implementation patterns in `packages/ui/src/components/Form` as the reference (props, slots, emits, description).
